...
library(ggsci)
woof = display.brewer.pal(10, "Pastel2")

library(RColorBrewer)
#color_list = c("medium sea green","yellow green","gold")
#color_list = c("#58D68D","yellow green", "#F1FF45")
...
col2rgb("medium sea green")
col2rgb("yellow green")
col2rgb("gold")

plot(c(1,2,3), c(10, 5, 1), xlim = c(1, 3), ylim = c(0,10))
arrows(1, 10, 2, 5)
arrows(2, 5, 3, 1)
...
blat_info = read.csv("~/RNA_Seq/merged_outfiles.psl_forlongesti.longesti", sep = "\t", header = FALSE)
...
PCA_all = plotPCA12(vst_all, intgroup1 = "Cond", intgroup2 = "Ecotype", intgroup3 = "Geog", ntop = 1000000, my_xlims = c(-280, 150),my_ylims = c(-100, 120), my_background_col = "black", thicko = 4)
PCA_all
...
pdf(file = "Fig2A_200721_PCA_all_centroids2.pdf")
...
vst_Ci = vst_all[,vst_all$Cond %in% "C"]
PCA_control_plot = plotPCA12(vst_Ci, my_xlims = c(-100,150), my_ylims = c(-100,75), intgroup1 = "Cond", intgroup2 = "Ecotype", intgroup3 = "Geog", ntop = 2000000, my_background_col = "black", thicko = 4)
PCA_control_plot



PCA_control_plot = plotPCA12(vst_C, intgroup1 = "Cond", intgroup2 = "Ecotype", intgroup3 = "Geog", ntop = 2000000,  my_background_col = "black", thicko = 4)
PCA_control_plot
...
#Fig 1B
venn.diagram(x= list(resB_sig$Names, resC_sig$Names), category.names = c("", ""), filename = "VennDiagram_BvC_140820.png", output = TRUE, cat.pos = c(0,0), rotation.degree = 180, height = 2000, width = 2000, imagetype = "png") 
...
#Black and white for these guys I think, just to make it easier...
...
##### Plot Figure 2 - 24/07/21
#Man this looks awful. Sort this out at a later date. 
plot_grid(PCA_control_plot, resKresL_LFC_Plot, boxplot_resBresC_FC_220721, PCA_all_plot)
...
quantile(resids_countsH_resKresL)
...
norm_counts_cut$logPC = norm_counts_cut$PC*log2(abs(norm_counts_cut$PC))/abs(norm_counts_cut$PC)
head(norm_counts_cut$logPC)
head(norm_counts_cut$PC)
norm_counts_cut$logGC = norm_counts_cut$GC*log2(abs(norm_counts_cut$GC))/abs(norm_counts_cut$GC)

dim(norm_counts_cut)[1]/dim(norm_counts)[1]
...
norm_counts_cut_G1 = norm_counts[norm_counts$Cutoff_GC_G1 == "Y" & norm_counts$Cutoff_PC_G1 == "Y",]
norm_counts_cut_G2 = norm_counts[norm_counts$Cutoff_GC_G2 == "Y" & norm_counts$Cutoff_PC_G2 == "Y",]

norm_counts_cut_G1$DirStat = norm_counts_cut_G1$GC_G1*norm_counts_cut_G1$PC_G1
norm_counts_cut_G2$DirStat = norm_counts_cut_G2$GC_G2*norm_counts_cut_G2$PC_G2

norm_counts_cut_G1$Type = ifelse(norm_counts_cut_G1$DirStat > 0, "RI", ifelse(abs(norm_counts_cut_G1$GC_G1) > 0.5*abs(norm_counts_cut_G1$PC_G1), "REV", "OVERSHOOT"))
norm_counts_cut_G2$Type = ifelse(norm_counts_cut_G2$DirStat > 0, "RI", ifelse(abs(norm_counts_cut_G2$GC_G2) > 0.5*abs(norm_counts_cut_G2$PC_G2), "REV", "OVERSHOOT"))

100*table(norm_counts_cut_G1$Type)[1]/nrow(norm_counts_cut_G1) #% overshooting
100*table(norm_counts_cut_G1$Type)[2]/nrow(norm_counts_cut_G1) #% reversion
100*table(norm_counts_cut_G1$Type)[3]/nrow(norm_counts_cut_G1) #% reinforcement

100*table(norm_counts_cut_G2$Type)[1]/nrow(norm_counts_cut_G2) #% overshooting
100*table(norm_counts_cut_G2$Type)[2]/nrow(norm_counts_cut_G2) #% reversion
100*table(norm_counts_cut_G2$Type)[3]/nrow(norm_counts_cut_G2) #% reinforcement


100*dim(norm_counts_cut_G1)[1]/dim(norm_counts)[1]
100*dim(norm_counts_cut_G2)[1]/dim(norm_counts)[1]
...
100*dim(norm_counts_cut)[1]/dim(norm_counts)[1]
...
table(norm_counts_cut$Type)[1]
table(norm_counts_cut$Type)[2]
table(norm_counts_cut$Type)[3]
...
norm_counts_resKresL = norm_counts_cut[rownames(norm_counts_cut) %in% resKresL_sig_overlap_samedir$Names,]
dim(norm_counts_resKresL)[1]/dim(resKresL_sig_overlap_samedir)[1]
...
table(norm_counts_resKresL$Type)[2]
table(norm_counts_resKresL$Type)[1]
table(norm_counts_resKresL$Type)[3]
...
norm_counts_resKresL$PC_pc = norm_counts_resKresL$PC/norm_counts_resKresL$Lp
norm_counts_resKresL$GC_pc = norm_counts_resKresL$GC/norm_counts_resKresL$Lp
norm_counts_resBresC$PC_pc = norm_counts_resBresC$PC/norm_counts_resBresC$Lp
norm_counts_resBresC$GC_pc = norm_counts_resBresC$GC/norm_counts_resBresC$Lp

ggplot(data = norm_counts_resKresL, aes(log2(PC), log2(GC)))+geom_bin2d(bins=100)+xlim(-10,10)+ylim(-10,10)
ggplot(data = norm_counts_resBresC, aes(PC_pc, GC_pc))+geom_bin2d(bins=100)
...
norm_counts_cut_G1_resKnotL = norm_counts_cut_G1[rownames(norm_counts_cut_G1) %in% resK_notL$Names,]
norm_counts_cut_G2_resLnotK = norm_counts_cut_G2[rownames(norm_counts_cut_G2) %in% resL_notK$Names,]

#i) what proportion of these genes don't meet the EC/PC cutoffs?
dim(norm_counts_cut_G1_resKnotL) #405 meet cutoff
dim(norm_counts_cut_G1_resKnotL)[1]/nrow(resK_notL) #So 83.5% pass both thresholds

dim(norm_counts_cut_G2_resLnotK) #1564
dim(norm_counts_cut_G2_resLnotK)/nrow(resL_notK) #So only 66% pass both thresholds.
...
norm_counts_G1_resKnotL = norm_counts[rownames(norm_counts) %in% resK_notL$Names,]
norm_counts_G2_resLnotK = norm_counts[rownames(norm_counts) %in% resL_notK$Names,]
dim(norm_counts_G1_resKnotL) #Same as before, good
dim(norm_counts_G2_resLnotK) #Same as before, good

#So for G1...
table(norm_counts_G1_resKnotL$Cutoff_GC_G1) #So 437/485 pass the GC cutoff
table(norm_counts_G1_resKnotL$Cutoff_GC_G1)[2]/nrow(resK_notL) #so that's 90%
table(norm_counts_G1_resKnotL$Cutoff_PC_G1) #So 448/485.
table(norm_counts_G1_resKnotL$Cutoff_PC_G1)[2]/nrow(resK_notL) #so 92% pass PC as well. 

table(norm_counts_G2_resLnotK$Cutoff_GC_G2) #So 683/2365
table(norm_counts_G2_resLnotK$Cutoff_GC_G2)[2]/nrow(resL_notK) #...71.2% show substantial GC (i.e. derived genetic change)
table(norm_counts_G2_resLnotK$Cutoff_PC_G2) #So 3461/4035, or...
table(norm_counts_G2_resLnotK$Cutoff_PC_G2)[2]/nrow(resL_notK) #but still, 92.6% show substantail PC

#And how does this compare to the combined proportion?  
1-(dim(norm_counts_noPC_resKresL)[1]/nrow(resKresL_sig_overlap_samedir)) # So 98% fo the shared ones show substnail PC; a little higher.

#Ok so I think the real question is...for genes in resKnotL [G1 only}...do they show substantially lower anc. plast. in G2? I.e. is that why they're not DGE?
table(norm_counts_G1_resKnotL$Cutoff_PC_G2)[2]/nrow(resK_notL) #Nope...
table(norm_counts_G2_resLnotK$Cutoff_PC_G1)[2]/nrow(resL_notK) #Nope...

#So there isn't really strong evidence here that a lack of ancestral plasticity is driving recruitment.

norm_counts_cut_G1_resKresL = norm_counts_cut_G1[rownames(norm_counts_cut_G1) %in% resKresL_sig_overlap_samedir$Names,]
norm_counts_cut_G2_resKresL = norm_counts_cut_G2[rownames(norm_counts_cut_G2) %in% resKresL_sig_overlap_samedir$Names,]
dim(norm_counts_cut_G1_resKresL)[1]/nrow(resKresL_sig_overlap_samedir) #291:
dim(norm_counts_cut_G2_resKresL)[1]/nrow(resKresL_sig_overlap_samedir) #300:
...
100*table(norm_counts_cut_G1_resKnotL$Type)[1]/nrow(norm_counts_cut_G1_resKnotL) #% overshooting: 4.4
100*table(norm_counts_cut_G1_resKnotL$Type)[2]/nrow(norm_counts_cut_G1_resKnotL) #% reversion: 89.9%
100*table(norm_counts_cut_G1_resKnotL$Type)[3]/nrow(norm_counts_cut_G1_resKnotL) #% reinforcement; 5.7%
table(norm_counts_cut_G1_resKnotL$Type)[3] #So there are 23 reinforced genes in here...

100*table(norm_counts_cut_G1_resKresL$Type)[1]/nrow(norm_counts_cut_G1_resKresL) #% overshooting: 11.7%
100*table(norm_counts_cut_G1_resKresL$Type)[2]/nrow(norm_counts_cut_G1_resKresL) #% reversion: 69.8%
100*table(norm_counts_cut_G1_resKresL$Type)[3]/nrow(norm_counts_cut_G1_resKresL) #% reinforcement: 18.5%: so way more???

100*table(norm_counts_cut_G2_resLnotK$Type)[1]/nrow(norm_counts_cut_G2_resLnotK) #% overshooting 13.1%
100*table(norm_counts_cut_G2_resLnotK$Type)[2]/nrow(norm_counts_cut_G2_resLnotK) #% reversion 77.6%
100*table(norm_counts_cut_G2_resLnotK$Type)[3]/nrow(norm_counts_cut_G2_resLnotK) #% reinforcement; 9.27%

table(norm_counts_cut_G2_resLnotK$Type)[3] #And 290 reinforced genes in here.

100*table(norm_counts_cut_G2_resKresL$Type)[1]/nrow(norm_counts_cut_G2_resKresL) #% overshooting: 11.8%
100*table(norm_counts_cut_G2_resKresL$Type)[2]/nrow(norm_counts_cut_G2_resKresL) #% reversion: 71.6%
100*table(norm_counts_cut_G2_resKresL$Type)[3]/nrow(norm_counts_cut_G2_resKresL) #% reinforcement: 16.6%...

#So in the combined dataset, this is what...?
#A lot lower, right
...
norm_counts_resBresC = norm_counts_cut[rownames(norm_counts_cut) %in% resBresC_sig_overlap_samedir$Names,]
dim(norm_counts_resBresC)[1]/nrow(resBresC_sig_overlap_samedir)


100*table(norm_counts_resBresC$Type)[1]/(table(norm_counts_resBresC$Type)[1]+table(norm_counts_resBresC$Type)[2]+table(norm_counts_resBresC$Type)[3]) #% overshooting
100*table(norm_counts_resBresC$Type)[2]/(table(norm_counts_resBresC$Type)[1]+table(norm_counts_resBresC$Type)[2]+table(norm_counts_resBresC$Type)[3]) #% reversion
100*table(norm_counts_resBresC$Type)[3]/(table(norm_counts_resBresC$Type)[1]+table(norm_counts_resBresC$Type)[2]+table(norm_counts_resBresC$Type)[3]) #% reinforcement
...
norm_counts_cut_G1_resBnotC = norm_counts_cut_G1[rownames(norm_counts_cut_G1) %in% resB_notC$Names,]
norm_counts_cut_G2_resCnotB = norm_counts_cut_G2[rownames(norm_counts_cut_G2) %in% resC_notB$Names,]
...
#And how does this compare to the combined proportion?  
1-(dim(norm_counts_noPC_resBresC)[1]/nrow(resBresC_sig_overlap_samedir)) # So 82.8% showed substantial PC in ancestral populations (in both?)
...
norm_counts_G1_resBnotC$PC_G1G2_samedir = norm_counts_G1_resBnotC$PC_G1*norm_counts_G1_resBnotC$PC_G2
#So...this is getting pretty confusing, isn't it. 

hist(norm_counts_G1_resBnotC$PC_G1G2_samedir)
hist(relative_BnoC, breaks = 4000, xlim = c(-10,10))

#So there isn't really strong evidence here that a lack of ancestral plasticity is driving recruitment.

norm_counts_cut_G1_resBresC = norm_counts_cut_G1[rownames(norm_counts_cut_G1) %in% resBresC_sig_overlap_samedir$Names,]
norm_counts_cut_G2_resBresC = norm_counts_cut_G2[rownames(norm_counts_cut_G2) %in% resBresC_sig_overlap_samedir$Names,]
...
100*table(norm_counts_G1_resBresC$Type)[1]/nrow(norm_counts_G1_resBresC) #% overshooting: 4.46% 
100*table(norm_counts_G1_resBresC$Type)[2]/nrow(norm_counts_G1_resBresC) #% reversion: 60.1%
100*table(norm_counts_G1_resBresC$Type)[3]/nrow(norm_counts_G1_resBresC) #% reinforcement: 35.4%
...
100*table(norm_counts_G2_resBresC$Type)[1]/nrow(norm_counts_G2_resBresC) #% overshooting: 1.7%
100*table(norm_counts_G2_resBresC$Type)[2]/nrow(norm_counts_G2_resBresC) #% reversion: 67%
100*table(norm_counts_G2_resBresC$Type)[3]/nrow(norm_counts_G2_resBresC) #% reinforcement: 31.3%


################################################
#### |FC| values for different classes - AP, CEC
###############################################
...
dim(norm_counts_noPC_resBresC)# So Out of 371, 
...
quantile(temp_Plast_FC_BC$VALUE)
quantile(resids_countsH_BC_NOANCPLAST$VALUE)
boxplot(temp_Plast_FC_BC$VALUE, ylim = c(0,1))
boxplot(resids_countsH_BC_NOANCPLAST$VALUE, ylim = c(0,2))
quantile(resids_countsH_BC_NOANCPLAST$VALUE)
quantile(temp_Plast_FC_BC$VALUE)


boxplot(PlastNoPlast_FC_BC$VALUE ~ PlastNoPlast_FC_BC$Names, ylim = c(0,1))
table(PlastNoPlast_FC_BC$Names)
Plast_FC_BC_wilcox = wilcox.test(resids_countsH_BC_NOANCPLAST$VALUE, temp_Plast_FC_BC$VALUE, alternative = "two.sided")
Plast_FC_BC_wilcox
...
ggplot()+  geom_boxplot(data = PlastNoPlast_FC_BC,fill = another_plot_purple, aes(x = Names, y=VALUE), width = 0.7, outlier.shape = NA, show.legend = FALSE)+ylim(0,1)
...
KL_PlastNoPlast_Boxplot = ggplot()+
  ...
  scale_x_discrete(labels = c("NP", "P"))+
  #facet_grid(.~Set)+
  xlab("")+ylab("Absolute log2 fold change (|FC|)")+coord_cartesian(ylim=c(0,0.5))+
  #theme(panel.background = element_rect(fill = "lemonchiffon1"))
  theme(panel.background = element_rect(fill = NA, colour = "black", size = 1, linetype = c("solid", "solid")), 
        text = element_text(size = 30),
        strip.text = element_text(size = 30))
...
plot_grid(T_barplot, KL_barplot, BC_barplot, nrow = 3, ncol = 1)
...
#Need to add a background then...

#So we need some background...

#AP genes
ap_lim = max(c(quantile(norm_counts_resKresL$PC)[1], quantile(norm_counts_resKresL$PC)[5], quantile(norm_counts_resKresL$GC)[1], quantile(norm_counts_resKresL$GC)[5]))
ggplot(data = norm_counts_resKresL, aes(PC, GC))+geom_bin2d(bins=100)+xlim(-biggest_lim, biggest_lim)+
  ylim(-biggest_lim, biggest_lim)+geom_abline(slope = -0.5, intercept = 0)+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+theme(text=element_text(size=15))


#Middle 50%+ of the data...
ap_lim = max(c(quantile(norm_counts_resKresL$PC)[2], quantile(norm_counts_resKresL$PC)[4], quantile(norm_counts_resKresL$GC)[2], quantile(norm_counts_resKresL$GC)[4]))
ggplot(data = norm_counts_resKresL, aes(PC, GC))+geom_bin2d(bins=100)+xlim(-biggest_lim, biggest_lim)+
  ylim(-ap_lim, ap_lim)+geom_abline(slope = -0.5, intercept = 0)+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+theme(text=element_text(size=15))

#CEC genes
cec_lim = max(c(quantile(norm_counts_resBresC$PC)[1], quantile(norm_counts_resBresC$PC)[5], quantile(norm_counts_resBresC$GC)[1], quantile(norm_counts_resBresC$GC)[5]))
ggplot(data = norm_counts_resBresC, aes(x = PC, y = GC))+geom_bin2d(bins=40)+xlim(-biggest_lim, biggest_lim)+
  ylim(-biggest_lim, biggest_lim)+geom_abline(slope = -0.5, intercept = 0)+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+theme(text=element_text(size=15))

ggplot(data = norm_counts_resBresC, aes(x = PC, y = GC))+geom_bin2d(bins=10)+geom_abline(slope = -0.5, intercept = 0)+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+theme(text=element_text(size=15))+xlim(-biggest_lim, biggest_lim)+
  ylim(-biggest_lim, biggest_lim)

temp = norm_counts_resBresC[norm_counts_resBresC$GC > 0,]
boxplot(temp$Lo, ylim = c(0,400))
boxplot(temp$Lp, ylim = c(0,400))
boxplot(temp$La, ylim = c(0,400))

dim(temp)
temp
ggplot(data = temp, aes(x = PC, y = GC))+geom_bin2d(bins=100)+geom_abline(slope = -0.5, intercept = 0)+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+theme(text=element_text(size=15))+xlim(-biggest_lim, biggest_lim)+
  ylim(-biggest_lim, biggest_lim)



boxplot(temp$PC, ylim = c(-50, 50))
boxplot(temp$GC, ylim = c(-50, 50))


#Middle 50%+ of the data...
cec_lim = max(c(quantile(norm_counts_resBresC$PC)[2], quantile(norm_counts_resBresC$PC)[4], quantile(norm_counts_resBresC$GC)[2], quantile(norm_counts_resBresC$GC)[4]))
ggplot(data = norm_counts_resBresC, aes(PC, GC))+geom_bin2d(bins=75)+xlim(-cec_lim, cec_lim)+
  ylim(-biggest_lim, biggest_lim)+geom_abline(slope = -0.5, intercept = 0)+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+theme(text=element_text(size=15))


par(mfrow = c(1,2))
hist(norm_counts_cut$PC, breaks = 10000, xlim = c(-50,50))
hist(norm_counts_cut$GC, breaks = 10000, xlim = c(-50,50))

temp_x = c(log2(1), log2(10),log2(1000), log2(10000),-log2(1), -log2(10),-log2(1000), -log2(10000))
temp_y = c(-log2(0.5), -log2(5),-log2(500), -log2(5000),log2(0.5), log2(5),log2(500), log2(5000))
df_test = data.frame(temp_x, temp_y)
plot(c(10,20,30,40), c(-5,-10,-15,-20), xlim = c(-40, 40), ylim = c(-40,40))

quantile(norm_counts_cut$PC, probs = c(0.025, 0.975))
quantile(norm_counts_cut$GC, probs = c(0.025, 0.975))
ggplot(data = norm_counts_cut, aes(PC, GC))+geom_bin2d(bins=50)+xlab("PC")+ylab("EC")+xlim(-1355, 1355)+ylim(-1355,1355)+
  geom_abline(slope = -0.5, intercept = 0)+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)

ggplot(data = norm_counts_cut, aes(logPC, logGC))+geom_bin2d(bins=50)+xlab("PC")+ylab("EC")+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+geom_point(data = df_test, aes(temp_x,temp_y))+geom_smooth()

par(mfrow = c(1,1))  
hist(norm_counts_cut$PC/norm_counts_cut$GC, breaks = c(1000), xlim = c(-10,10))

quantile(norm_counts_resBresC$PC, probs = c(0.025, 0.975))
quantile(norm_counts_resBresC$GC, probs = c(0.025, 0.975))

ggplot(data = norm_counts_resBresC, aes(PC, GC))+geom_bin2d(bins=50)+xlab("PC")+ylab("EC")+xlim(-10700, 10700)+ylim(-10700,10700)+
  geom_abline(slope = -0.5, intercept = 0)+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)



ggplot(data = norm_counts_cut, aes(PC, GC))+geom_bin2d(bins=200)+xlab("log10(PC)")+ylab("log10(GC")
ggplot(data = norm_counts_resKresL, aes(PC, GC))+geom_bin2d(bins=200)+xlab("log10(PC)")+ylab("log10(GC")
ggplot(data = norm_counts_resBresC, aes(PC, GC))+geom_bin2d(bins=100)+xlab("PC")+ylab("EC")+geom_abline(slope = -0.5, intercept = 0)+xlim(-1000,1000)+ylim(-1000,1000)
ggplot(data = norm_counts_resKresL, aes(PC, GC))+geom_bin2d(bins=100)+xlab("PC")+ylab("EC")+geom_abline(slope = -0.5, intercept = 0)+xlim(-3000,3000)+ylim(-3000,3000)
iwr

#Right, ok.

ggplot(data = norm_counts_cut, aes(logPC, logGC))+geom_bin2d(bins=200)+xlab("log10(PC)")+ylab("log10(GC")


#Reinforcement:

DirStat > 0 [changes in same dir]
...
#Middle 50%+ of the data...
ncc_lim = max(c(quantile(norm_counts_cut$PC)[2], quantile(norm_counts_cut$PC)[4], quantile(norm_counts_cut$GC)[2], quantile(norm_counts_cut$GC)[4]))
ap_lim = max(c(quantile(norm_counts_resKresL$PC)[2], quantile(norm_counts_resKresL$PC)[4], quantile(norm_counts_resKresL$GC)[2], quantile(norm_counts_resKresL$GC)[4]))
cec_lim = max(c(quantile(norm_counts_resBresC$PC)[2], quantile(norm_counts_resBresC$PC)[4], quantile(norm_counts_resBresC$GC)[2], quantile(norm_counts_resBresC$GC)[4]))
...
Middle_ECGC = ggplot(data = data.frame())+geom_point()+xlim(-biggest_lim, biggest_lim)+ylim(-biggest_lim, biggest_lim)+
  ...
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
  geom_bin2d(data = norm_counts_cut, aes(PC, GC), bins=50)+
...
WholeAP_ECGC = ggplot(data = data.frame())+geom_point()+
  ...
  #geom_polygon(data = over4, aes(x=x, y=y), fill = color_list[3])+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
  geom_bin2d(data = norm_counts_resKresL, aes(PC, GC), bins=50)+
  geom_abline(slope = -0.5, intercept = 0, col = "black")+
  geom_abline(slope = -1, intercept = 0, lty = 2, col = "black")+
  theme(text = element_text(size = 20))+xlab("Plastic Change (PC)")+ylab("Evolutionary Change (EC)")
WholeAP_ECGC
...
WholeCEC_ECGC = ggplot(data = data.frame())+geom_point()+
  ...
  #geom_polygon(data = over4, aes(x=x, y=y), fill = color_list[3])+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
  geom_bin2d(data = norm_counts_resBresC, aes(PC, GC), bins=50)+
  geom_abline(slope = -0.5, intercept = 0, col = "black")+
  geom_abline(slope = -1, intercept = 0, lty = 2, col = "black")+
  theme(text = element_text(size = 20))+xlab("Plastic Change (PC)")+ylab("Evolutionary Change (EC)")
WholeCEC_ECGC
...
AP_ECGC = ggplot(data = data.frame())+geom_point()+xlim(-biggest_lim, biggest_lim)+ylim(-biggest_lim, biggest_lim)+
  ...
  geom_abline(slope = -1, intercept = 0, lty = 2)+geom_bin2d(data = norm_counts_resKresL, aes(PC, GC), bins=50)+
...
ggplot(data = data.frame())+geom_point()+xlim(-biggest_lim, biggest_lim)+ylim(-biggest_lim, biggest_lim)+
  geom_rect(aes(xmin = -Inf, xmax  = 0, ymin = -Inf, ymax = 0), fill=color_list[1])+
  geom_rect(aes(xmin = Inf, xmax  = 0, ymin = Inf, ymax = 0), fill=color_list[1])+
  geom_polygon(data = over1, aes(x=x, y=y), fill = color_list[2])+
  geom_polygon(data = over2, aes(x=x, y=y), fill = color_list[2])+
  ...
  geom_polygon(data = over4, aes(x=x, y=y), fill = color_list[3])+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
  geom_bin2d(data = norm_counts_resBresC, aes(PC, GC), bins=75)+
  geom_abline(slope = -0.5, intercept = 0)+
...
geom_rect(aes(xmin = 0.45, xmax = 2.5, ymin = -Inf, ymax = Inf),alpha = 0.6, fill = col_coast)+
  ...
  over1 = data.frame(x = c(-10.5,-10.5, 0, 0), y = c(0,-10.5,-10.5,0))
ggplot(data = norm_counts_cut, aes(PC, GC))+geom_bin2d(bins=150)+xlim(-biggest_lim, biggest_lim)+
  ylim(-biggest_lim, biggest_lim)+geom_abline(slope = -0.5, intercept = 0)+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+theme(text=element_text(size=15))
