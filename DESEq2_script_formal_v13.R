#0 LOADNG IN THE DATA, FILTERING
  ...
  plotPCA12 <- function (object, intgroup1 = "condition", ntop = 2000000, returnData = FALSE, intgroup2 = "Geog", intgroup3 = "Ecotype", my_xlims = c(-200, 200), my_ylims = c(-200, 200), my_background_col = col_control, thicko = 8) 
  {
    ...
    ggplot(data = d, aes_string(x = "PC1", y = "PC2")) + xlim(my_xlims[1], my_xlims[2]) + ylim(my_ylims[1], my_ylims[2])+ 
      ...
      geom_point(size = 11, stroke=2, aes(shape = group1, col = group2, fill = group3)) + scale_shape_manual(values = c(21,24))+
      xlab(paste0("PC1: ", round(percentVar[1] * 100), "% variance")) + 
      scale_colour_manual(values = c(col_PPBD, col_GRSA)) + ylab(paste0("PC2: ", round(percentVar[2]*100), "% variance")) + scale_fill_manual(values = alpha(c(col_coast, col_mine), 0.7))  +
      
      coord_fixed() + theme(legend.position = "none")+
      theme(panel.border = element_rect(colour = my_background_col, fill = NA, size = thicko), panel.background = element_blank(), aspect.ratio = 1, axis.title = element_text(size = 27), axis.text = element_text(size = 20)) #, panel.grid.major=element_line(colour="lightgrey"))
  ...
  plotPCA34 <- function (object, intgroup1 = "Geog", ntop = 2000000, returnData = FALSE, intgroup2 = "Ecotype", intgroup3 = "condition", my_xlims = c(-200, 200), my_ylims = c(-200, 200), my_background_col = col_control, thicko = 8) 
  {
    ...
    ggplot(data = d, aes_string(x = "PC3", y = "PC4")) + xlim(my_xlims[1], my_xlims[2]) + ylim(my_ylims[1], my_ylims[2])+ 
      ...
      geom_point(size = 8, stroke=2, aes(shape = group1, col = group3, fill = group2)) + scale_shape_manual(values = c(21,24)) + xlab(paste0("PC3: ", round(percentVar[3] * 
                                                                                                                                                              100), "% variance")) + 
      scale_colour_manual(values = c(col_PPBD, col_GRSA)) + ylab(paste0("PC4: ", round(percentVar[4]*100), "% variance")) + scale_fill_manual(values = alpha(c(col_coast, col_mine), 0.7)) +
  ...
  plotPCA56 <- function (object, intgroup1 = "Geog", ntop = 2000000, returnData = FALSE, intgroup2 = "Ecotype", intgroup3 = "condition", my_xlims = c(-200, 200), my_ylims = c(-200, 200), my_background_col = col_control, thicko = 8) 
  {
    rv <- rowVars(assay(object))
    select <- order(rv, decreasing = TRUE)[seq_len(min(ntop, 
                                                       length(rv)))]
    ...
    d <- data.frame(PC5 = pca$x[, 5], PC6 = pca$x[, 6], 
                    group1 = group1, group2 = group2, group3 = group3, name = colnames(object))
    intgrop3d <- data.frame(PC5 = pca$x[, 5], PC6 = pca$x[, 6], group1 = group1, 
                            intgroup1.df, group2 = group2, intgroup2.df, name = colnames(object))
    ...
    ggplot(data = d, aes_string(x = "PC5", y = "PC6")) + xlim(my_xlims[1], my_xlims[2]) + ylim(my_ylims[1], my_ylims[2])+ 
      #    ggplot(data = d, aes_string(x = "PC1", y = "PC2"))+ 
      geom_point(size = 8, stroke=2, aes(shape = group1, col = group3, fill = group2)) + scale_shape_manual(values = c(21,24)) + xlab(paste0("PC5: ", round(percentVar[5] * 
                                                                                                                                                              100), "% variance")) + 
      scale_colour_manual(values = c(col_PPBD, col_GRSA)) + ylab(paste0("PC6: ", round(percentVar[6]*100), "% variance")) + scale_fill_manual(values = alpha(c(col_coast, col_mine), 0.7))  +
  ...
  old_emb50plus = emb_yes[emb_yes$V4 > 50,]
  old_blat_latifolia_info = read.csv("~/RNA_Seq/merged_outfiles.psl_forlongesti.longesti", sep = "\t", header = FALSE)
  old_blat_latifolia_info$match_length = old_blat_latifolia_info$V1+old_blat_latifolia_info$V2
  old_blat_match_50plus = old_blat_latifolia_info[old_blat_latifolia_info$match_length > 50,]
  old_blat_50plus_list = old_blat_match_50plus$V10
  head(old_blat_50plus_list)
  old_GC_content_Filter2 = GC_content[GC_content$V1 %in% old_blat_50plus_list | GC_content$V1 %in% old_emb50plus$V1,]
  dim(old_GC_content_Filter2)
  par(mfrow = c(1,1))  
  
dev.new(width = 550, height = 550)
par(mar=c(8,8,8,8))
  hist(GC_content$V2, breaks = 80, xlim = c(20,80), main = "Unfiltered", xlab = "GC content (%)", cex.axis = 1.5, cex.lab = 2, cex.main = 2.5)
  hist(old_GC_content_Filter2$V2, breaks = 80, xlim = c(20,80), main = "Previous Filtering", xlab = "GC content (%)", cex.axis = 1.5, cex.lab = 2, cex.main = 2.5)
  hist(GC_filter3$V2, breaks = 80, xlim = c(20,80), main = "New filtering", xlab = "GC content (%)", cex.axis = 1.5, cex.lab = 2, cex.main = 2.5) #So this still has a load of crap
  ...
  sr30 = read.csv("~/sorted_merged_outfile_best.psl.expr.filt.re.sorted.not.SR30", header = FALSE, sep = "\t")
  sr30 = na.omit(sr30)
  
  dim(sr30)
  hist(sr30$V2, xlab = "Percentage match length on different scaffold", main = "Multimapping contigs")
  head(sr30)
  dim(sr30)
  ...
  vst_C = vst(dds_C) #I think this way of doing it is fine: you use the variance smoothing of the relevant conditions.
  #For a small number of genes, better to use the whole set of genes. 
  #Maybe run this once done?   
  vst_C = vst_all[,vst_all$Cond %in% "C"]
  dim(vst_C)
...
pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/SP2.pdf", width = 6.42, height = 5.84)
vst_C_plot = vst_C
vst_C_plot
#Note: obviously this is wrong but makes plotting easier
vst_C_plot$Pop_Cond = levels(as.factor(c("S2i", "S2ii", "S2iii", "T1i", "T1ii", "T1iii", "T2i", "T2ii", "T2iii", "S1i", "S1ii", "S1iii")))
vst_C_plot$Pop_Cond
heatmap_samples(vst_C_plot)
dev.off()
heatmap_samples(vst_all)
  ...
  PCA_all = plotPCA12(vst_all, intgroup1 = "Cond", intgroup2 = "Geog", intgroup3 = "Ecotype", ntop = 1000000, my_xlims = c(-200, 100),my_ylims = c(-100, 100), my_background_col = "black", thicko = 4)
  PCA_all
  ...
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/P1D.pdf", width = 1.5*5.3, height = 1.5*5.84)
  
  PCA_all_plot3 = PCA_all+
    geom_segment(aes(x = pca.centroids[7,2], y = pca.centroids[7,3], xend = pca.centroids[8,2], yend = pca.centroids[8,3]), linetype = "dashed",  size = 1, col = "black", lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids[1,2], y = pca.centroids[1,3], xend = pca.centroids[2,2], yend = pca.centroids[2,3]), linetype = "dashed",  size = 1,col = "black", lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids[8,2], y = pca.centroids[8,3], xend = pca.centroids[4,2], yend = pca.centroids[4,3]), arrow = arrow(length = unit(0.03, "npc")), size = 1, col = "black", lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids[2,2], y = pca.centroids[2,3], xend = pca.centroids[6,2], yend = pca.centroids[6,3]), arrow = arrow(length = unit(0.03, "npc")), size = 1, col = "black", lineend = "round", linejoin = "round") #Not sure exactly how to add these? 
  PCA_all_plot3
  dev.off()
  
  
  PCA_all_plot2 = PCA_all+
    geom_segment(aes(x = pca.centroids[7,2], y = pca.centroids[7,3], xend = pca.centroids[8,2], yend = pca.centroids[8,3]), arrow = arrow(length = unit(0.03, "npc")), size = 2.5, col = "black", lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids[7,2], y = pca.centroids[7,3], xend = pca.centroids[8,2], yend = pca.centroids[8,3]), arrow = arrow(length = unit(0.03, "npc")), size = 1.5, col = coast_plot, lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids[1,2], y = pca.centroids[1,3], xend = pca.centroids[2,2], yend = pca.centroids[2,3]), arrow = arrow(length = unit(0.03, "npc")), size = 2.5,col = "black", lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids[1,2], y = pca.centroids[1,3], xend = pca.centroids[2,2], yend = pca.centroids[2,3]), arrow = arrow(length = unit(0.03, "npc")), size = 1.5, col = coast_plot,  lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids[8,2], y = pca.centroids[8,3], xend = pca.centroids[4,2], yend = pca.centroids[4,3]), arrow = arrow(length = unit(0.03, "npc")), size = 2.5, col = "black", lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids[8,2], y = pca.centroids[8,3], xend = pca.centroids[4,2], yend = pca.centroids[4,3]), arrow = arrow(length = unit(0.03, "npc")), size = 1.5, col = mine_plot, lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids[2,2], y = pca.centroids[2,3], xend = pca.centroids[6,2], yend = pca.centroids[6,3]), arrow = arrow(length = unit(0.03, "npc")), size = 2.5, col = "black", lineend = "round", linejoin = "round")+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids[2,2], y = pca.centroids[2,3], xend = pca.centroids[6,2], yend = pca.centroids[6,3]), arrow = arrow(length = unit(0.03, "npc")), size = 1.5, col = mine_plot, lineend = "round", linejoin = "round") #Not sure exactly how to add these? 
  #  geom_point(data =  pca_centroids_c, size = 4, shape = 21, fill = c(PPBD_plot, GRSA_plot, PPBD_plot, GRSA_plot), alpha = 1)
  PCA_all_plot2
  dev.off()
  ...
  #KR7
  PCA_control_plot = plotPCA12_centroids(vst_C, intgroup1 = "Cond", my_xlims = c(-50, 50), my_ylims = c(-50,50), intgroup2 = "Ecotype", intgroup3 = "Geog", ntop = 2000000,  my_background_col = "black", thicko = 4)
  blork_C_all = as.data.frame(PCA_control_plot$x)
  blork_C_all$groups = vst_C$Pop
  pca.centroids_C_all = aggregate(blork_C_all[,1:2], list(Type = blork_C_all$groups), mean)
  pca.centroids_C_all
  par(mfrow = c(1,1))
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/P1B.pdf", width = 1.5*5.3, height = 1.5*5.84)
  PCA_control_plot = plotPCA12(vst_C, intgroup1 = "Cond", my_xlims = c(-100, 100), my_ylims = c(-100,100), intgroup2 = "Geog", intgroup3 = "Ecotype", ntop = 2000000,  my_background_col = "black", thicko = 4)
  pca.centroids_C_all
  PCA_control_plot+
    geom_segment(aes(x = pca.centroids_C_all[1,2], y = pca.centroids_C_all[1,3], xend = pca.centroids_C_all[3,2], yend = pca.centroids_C_all[3,3]), arrow = arrow(length = unit(0.03, "npc")), size = 1, col = "black", lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids_C_all[4,2], y = pca.centroids_C_all[4,3], xend = pca.centroids_C_all[2,2], yend = pca.centroids_C_all[2,3]), arrow = arrow(length = unit(0.03, "npc")), size = 1,col = "black", lineend = 'round', linejoin = 'round') #Not sure exactly how to add these? 
  dev.off()
  
  PCA_control_plot
  
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/SP7A.pdf", width = 1.5*5.3, height = 1.5*5.84)
  PCA_control_34 = plotPCA34(vst_C, intgroup1 = "Cond", my_xlims = c(-75, 100), my_ylims = c(-75,75), intgroup2 = "Ecotype", intgroup3 = "Geog", ntop = 2000000,  my_background_col = "black", thicko = 4)
  PCA_control_34
  dev.off()
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/SP7B.pdf", width = 1.5*5.3, height = 1.5*5.84)
  PCA_control_56 = plotPCA56(vst_C, intgroup1 = "Cond", my_xlims = c(-100, 75), my_ylims = c(-100,100), intgroup2 = "Ecotype", intgroup3 = "Geog", ntop = 2000000,  my_background_col = "black", thicko = 4)
  PCA_control_56
  dev.off()
  ...
  #83.3%
  (dim(resKresL_DPN_sig_overlap_samedir)[1]-dim(resIresJ_resKresLDPN)[1])
  (dim(resKresL_DPN_sig_overlap_samedir)[1]-dim(resIresJ_resKresLDPN)[1])/dim(resKresL_DPN_sig_overlap_samedir)[1]
  ...
  library(grDevices)
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/S2D_VennDiagram_BC.pdf")
  grid.draw(Fig2B)
  dev.off()
  ...
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/S2B_VennDiagram_FG.pdf")
  grid.draw(Fig2D)
  dev.off()
  ...
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/S2A_VennDiagram_IJKL.pdf")
  grid.draw(Fig3A)
  dev.off()
  ...
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/S2C_Venn_IJKLFG.pdf")
  grid.draw(Fig2F)
  dev.off()
  ...
  #KR86: Genetic assimilation.
  dim(resBresC_sig_overlap_samedir) #402 CEC genes. How many are not plastic in both.
  resBresC_sig_overlap_samedir_notplast = resBresC_sig_overlap_samedir[resBresC_sig_overlap$Names %notin% resK_sig$Names & resBresC_sig_overlap$Names %notin% resJ_sig$Names,]
  dim(resBresC_sig_overlap_samedir_notplast) #249 genes not DE in either tolerant population. 
  #So then we want to know...how many of these genes have substantial PC but not substantial EC?
  norm_CEC_noplast_Y = norm_counts[rownames(norm_counts) %in% resY$Names & rownames(norm_counts) %in% resBresC_sig_overlap_samedir_notplast$Names,]
  dim(norm_CEC_noplast_Y) #So 190 of these had substantial ancestral plasticity. 
  norm_CEC_noplast_YnoX = norm_CEC_noplast_Y[rownames(norm_CEC_noplast_Y) %notin% resX_sig$Names,]
  dim(norm_CEC_noplast_YnoX) #So of 249 fixed CEC genes, 190 have ancestral plasticity; 60 have substantial ancestral plasticity but no substantial EC
  write.csv(file = "norm_CEC_noplast_YnoX.csv", x = rownames(norm_CEC_noplast_YnoX))
  #So how many of the nonplastic ones display overshooting/reversion?
  norm_counts_XY_resBresC_noderivedplast = norm_counts_XY_resBresC[rownames(norm_counts_XY_resBresC) %in% rownames(norm_CEC_noplast_Y),]
  dim(norm_counts_XY_resBresC_noderivedplast)
  table(norm_counts_XY_resBresC_noderivedplast$Type) 
  ...
  PCA_resKresLDPN = plotPCA12(vst_resKresLDPN, intgroup1 = "Cond", intgroup2 = "Geog", intgroup3 = "Ecotype", ntop = 1000000, my_xlims = c(-35, 20),my_ylims = c(-15, 15), my_background_col = "black", thicko = 4)
  PCA_resKresLDPN
  ...
  pca.centroids_resKresLDPN
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/P1F.pdf", width = 1.5*5.3, height = 1.5*5.84)
  
  pca.centroids_resKresLDPN
  PCA_resKresLDPN_centroids_SupplementaryFigure = PCA_resKresLDPN+
    geom_segment(aes(x = pca.centroids_resKresLDPN[1,2], y = pca.centroids_resKresLDPN[1,3], xend = pca.centroids_resKresLDPN[2,2], yend = pca.centroids_resKresLDPN[2,3]), linetype = "dashed",  size = 1, col = "black", lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids_resKresLDPN[7,2], y = pca.centroids_resKresLDPN[7,3], xend = pca.centroids_resKresLDPN[8,2], yend = pca.centroids_resKresLDPN[8,3]), linetype = "dashed",  size = 1,col = "black", lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids_resKresLDPN[8,2], y = pca.centroids_resKresLDPN[8,3], xend = pca.centroids_resKresLDPN[4,2], yend = pca.centroids_resKresLDPN[4,3]), arrow = arrow(length = unit(0.03, "npc")), size = 1, col = "black", lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids_resKresLDPN[2,2], y = pca.centroids_resKresLDPN[2,3], xend = pca.centroids_resKresLDPN[6,2], yend = pca.centroids_resKresLDPN[6,3]), arrow = arrow(length = unit(0.03, "npc")), size = 1, col = "black", lineend = "round", linejoin = "round")+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids_resKresLDPN[3,2], y = pca.centroids_resKresLDPN[3,3], xend = pca.centroids_resKresLDPN[4,2], yend = pca.centroids_resKresLDPN[4,3]), linetype = "dashed",  size = 1, col = "black", lineend = 'round', linejoin = 'round')+ #Not sure exactly how to add these? 
    geom_segment(aes(x = pca.centroids_resKresLDPN[5,2], y = pca.centroids_resKresLDPN[5,3], xend = pca.centroids_resKresLDPN[6,2], yend = pca.centroids_resKresLDPN[6,3]), linetype = "dashed",  size = 1,col = "black", lineend = 'round', linejoin = 'round') #Not sure exactly how to add these? 
  PCA_resKresLDPN_centroids_SupplementaryFigure
  dev.off()
  ...
  Supplementary_Figure_1 = ggplot()+
    ...
    theme(panel.border = element_rect(colour = "black", fill = NA, size = 2), aspect.ratio = 1, panel.background = element_blank(), axis.text=element_text(size=15), axis.title = element_text(size=20))
  Supplementary_Figure_1
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/SP4A.pdf")
  ...
  #KR15
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/P1E.pdf", width = 1.5*6.6, height = 1.5*5.84)
  resKresLDPN_LFC_Plot = ggplot()+
    #xlim(-10,10)+ ylim(-10,10)+  
    #  geom_point(mapping = aes(x = log2FoldChange.x, y = log2FoldChange.y), data = lfcIvJ, col = "black")+
    geom_hline(yintercept = 0)+
    geom_vline(xintercept = 0)+
    geom_bin2d(mapping = aes(x = lfcKvL_resKresLDPN$log2FoldChange.x, y = lfcKvL_resKresLDPN$log2FoldChange.y), binwidth=0.5)+
    scale_fill_viridis_c()+
    xlim(-10, 10)+
    ylim(-10, 10)+
    geom_abline()+
    ...
    theme(text= element_text(size = 30), aspect.ratio=1, panel.border = element_rect(colour = "black", fill = NA, size = 5), panel.background = element_blank()) #, panel.grid.major=element_line(colour="lightgrey"))
  resKresLDPN_LFC_Plot
  dev.off()
  summary(lm(lfcKvL_resKresLDPN$log2FoldChange.y ~ lfcKvL_resKresLDPN$log2FoldChange.x))
  
  r<-max(abs(ggplot_build(resKresLDPN_LFC_Plot)$panel$ranges[[1]]$x.range))
  s<-max(abs(ggplot_build(resKresLDPN_LFC_Plot)$panel$ranges[[1]]$y.range))
  t<-round(max(r,s),1)
  resKresLDPN_LFC_Plot<-resKresLDPN_LFC_Plot+coord_equal(xlim=c(-t,t),ylim=c(-t,t))
  ...
  Fig5A_nocol = ggplot()+
    ...
    theme(axis.text.x = element_blank(), axis.title.y = element_text(size = 22), 
          axis.text.y = element_text(size = 16), aspect.ratio = 1, 
          panel.background = element_rect(fill = "white", colour = "black", size = 2),
          panel.border = element_rect(fill = "NA", colour = "black", size = 2))+
    xlab("")+ylab("log2(Absolute Fold Change) (|FC|)")+coord_cartesian(ylim=c(0,1))
  Fig5A_nocol
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/SP4B.pdf", width = 6.42, height = 5.84)
  ...
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/P1C.pdf", width = 1.5*6.19, height = 1.5*5.9)
  boxplot_resBresC_FC_220721 = ggplot()+
    ...
    theme(axis.text.x = element_blank(), axis.title.y = element_text(size = 35), axis.text.y = element_text(size = 25))+coord_cartesian(ylim=c(0,8.5))+
    ...
    theme(panel.background = element_rect(fill = NA), aspect.ratio=1, panel.border=element_rect(color="black", fill = NA, size = 5))
  boxplot_resBresC_FC_220721
  dev.off()
  
  # guess this is just the resA and resD comparison. Not used but useful to have. 
  boxplot_resBresC_FC_220721 = ggplot()+
  ...
  head(resids_CZ_resKresLDPN_eco)
  ...
  Supplementary_Figure_5A = ggplot()+
    ...
    xlab("")+ylab("log2(Absolute Fold Change) (|FC|)")+coord_cartesian(ylim=c(0,1.2))+
  ...
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/SP5.pdf")
  ...
  dim(norm_counts_cut_XY_G1)[1]/dim(norm_counts)[1]  
  dim(norm_counts_cut_XY_G2)[1]/dim(norm_counts)[1]  
  ...
  #PCAs...
  #KRWOOF
  vst_resKDPN_notLDPN = vst_all[rownames(vst_all) %in% resKDPN_notLDPN$Names,]
  vst_resLDPN_notKDPN = vst_all[rownames(vst_all) %in% resLDPN_notKDPN$Names,]
  PCA_resKDPN_notLDPN = plotPCA12(vst_resKDPN_notLDPN, intgroup1 = "Cond", intgroup2 = "Ecotype", intgroup3 = "Geog", ntop = 1000000, my_xlims = c(-25, 25),my_ylims = c(-10, 10), my_background_col = "lightgray")
  PCA_resKDPN_notLDPN
  PCA_resLDPN_notKDPN = plotPCA12(vst_resLDPN_notKDPN, intgroup1 = "Cond", intgroup2 = "Ecotype", intgroup3 = "Geog", ntop = 1000000, my_xlims = c(-45, 45),my_ylims = c(-20, 20), my_background_col = "lightgray")
  PCA_resLDPN_notKDPN
  ...
  dim(norm_counts_cut_XY_G2_resLDPN_notKDPN)[1]/nrow(resLDPN_notKDPN) #So only 66% pass both thresholds.
  ...
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/P3A1.pdf", width = 7.5, height = 3)
  T_barplot = ggplot(data = T_vec, aes(x = type, y = val, fill = type))+
    ...
    scale_fill_viridis_d()+theme(axis.text.y = element_text(size = 18),
                                 axis.text.x = element_text(size = 18), axis.title.x = element_text(size=19))+
    theme(panel.background = element_rect(fill = NA, colour = "black", size = 1, linetype = "solid"), 
          strip.text = element_text(size = 23))+facet_grid(.~title)+coord_flip()+scale_fill_manual(values = c(color_list[1], color_list[2], color_list[3]))
  
  T_barplot
  #Inkscape width = 86.707
  dev.off()
  
  #KR30 (1/2) - Barpolot for CEC genes in OVER, REV, RI
  
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/P3C1.pdf", width = 7.5, height = 3)
  ...
  BC_barplot = ggplot(data = BC_vec, aes(x = type, y = val, fill = type))+
    ...
    scale_fill_viridis_d()+theme(axis.text.y = element_text(size = 18),
                                 axis.text.x = element_text(size = 18), axis.title.x = element_text(size=19))+
    theme(panel.background = element_rect(fill = NA, colour = "black", size = 1, linetype = "solid"), 
          strip.text = element_text(size = 23))+facet_grid(.~title)+coord_flip()+scale_fill_manual(values = color_list)
  ...
  #KR26 (1/2) - Fig3B
  KL_vec$title = "DP genes"
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/P3B1.pdf", width = 7.5, height = 3)
  KL_barplot = ggplot(data = KL_vec, aes(x = type, y = val, fill = type))+
    geom_bar(width = 0.6, show.legend = FALSE, stat = 'identity', color = "black")+xlab("")+ylab("Number of genes\n")+
    scale_fill_viridis_d()+theme(axis.text.y = element_text(size = 18),
                                 axis.text.x = element_text(size = 18), axis.title.x = element_text(size=19))+
    theme(panel.background = element_rect(fill = NA, colour = "black", size = 1, linetype = "solid"), 
          strip.text = element_text(size = 23))+facet_grid(.~title)+coord_flip()+
  ...
  #KR37 (2/2)
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/P3D1.pdf", width = 3.1, height = 8)
  ...
  BC_PlastNoPlast_Boxplot = ggplot()+
    ...
    scale_x_discrete(labels = c("NP", "P"))+coord_cartesian(ylim = c(0,1.2))+
    #facet_grid(.~Set)+
    xlab("")+ylab("Absolute log2 fold change (|FC|)")+
    #theme(panel.background = element_rect(fill = "lemonchiffon1"))
    theme(panel.border = element_rect(fill = "NA", colour = "black", size = 2, linetype = c("solid", "solid", "solid", "solid")), 
          panel.background = element_rect(fill = "NA", colour = "black"), text = element_text(size = 24),
  ...
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/P3D2.pdf", width = 3.1, height = 8)
  KL_PlastNoPlast_Boxplot = ggplot()+
    ...
    scale_x_discrete(labels = c("NP", "P"))+
    #facet_grid(.~Set)+
    xlab("")+ylab("Absolute log2 fold change (|FC|)")+coord_cartesian(ylim=c(0,01))+
    #theme(panel.background = element_rect(fill = "lemonchiffon1"))
    theme(panel.border = element_rect(fill = NA, colour = "black", size = 2, linetype = c("solid", "solid")), 
  ...
  Whole_ECGC = ggplot(data = data.frame())+geom_point()+
    ...
    theme(text = element_text(size = 15), aspect.ratio=1, 
          ...
          panel.border=element_rect(fill = "NA", colour = "black", size = 2))+xlab("Plastic Change (PC)")+ylab("Evolutionary Change (EC)")
  Whole_ECGC
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/SP6A.pdf", width = 6.42, height = 5.84)
  grid.draw(Whole_ECGC)
  dev.off()
  
  
  #KR27
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/SP6B.pdf", width = 6.42, height = 5.84)
  WholeAP_ECGC = ggplot(data = data.frame())+geom_point()+
    ...
    aspect.ratio=1, panel.background=element_rect(fill = "NA", colour = "black", size = 2),
    panel.border=element_rect(fill = "NA", colour = "black", size = 2))+xlab("Plastic Change (PC)")+ylab("Evolutionary Change (EC)")
  WholeAP_ECGC
  dev.off()
  
  #KR31: Fig 5C
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/SP6C.pdf", width = 6.42, height = 5.84)  
...
dev.off()  
  #KR20 (2/2)
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/P3A2.pdf", width = 7.5, height = 7.5)
  Middle_ECGC = ggplot(data = data.frame())+geom_point()+xlim(-biggest_lim, biggest_lim)+ylim(-biggest_lim, biggest_lim)+
    ...
    theme(text = element_text(size = 15), aspect.ratio = 1, panel.border = element_rect(colour = "black", fill=NA, size=4))+xlab("Plastic Change (PC)")+ylab("Evolutionary Change (EC)")
  Middle_ECGC
  sdev.off()
  
  #KR26 (2/2) - Figure 3B
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/P3B2.pdf", width = 7.5, height = 7.5)
  ...
  pdf(file = "~/RNA_Seq/NEE_resubmission_030722/Figures/P3C2.pdf", width = 7.5, height = 7.5)
  ...
  classified_vec = rep(NA, nrow(norm_counts_cut_XY))
  head(classified_vec)
  ###############################################
  #Running the simulations. Note - this will take about an hour to run. 
  #Oh just sort this out tomorrow.
  
  #Run 30/06/22
  #for (i in 1:nrow(norm_counts_cut_XY)){
  #  Lo_row = norm_counts_cut_XY[i,c("BD11C", "BD12C", "BD1C", "SA6C", "SA7C", "SA8C")]
  #  Lp_row = norm_counts_cut_XY[i,c("BD11Z", "BD12Z", "BD1Z", "SA6Z", "SA7Z", "SA8Z")]
  #  La_row = norm_counts_cut_XY[i,c("GR10Z", "GR2Z", "GR8Z", "PP1Z", "PP4Z", "PP8Z")]
  #  classified_vec[i] = plasticity_parametric_bootstrap(Lo_row, Lp_row, La_row)
  #}
  table(classified_vec) #So 13k are unclassifiable. Was tha tthe case before?
  length(classified_vec)
  dim(norm_counts_cut_XY)
  norm_counts_cut_XY$temp_perm_calc = classified_vec
  
  write.csv(classified_vec, file = "final_param_classifications.csv")
  
  #Ugghhh wtf. 
  
  perm_table = table(norm_counts_cut_XY$temp_perm_calc)
  classifiable_perm = perm_table[1]+perm_table[2]+perm_table[3]
  
  #KR77
  perm_table[1]/classifiable_perm #OVER = 1.1% (81)
  perm_table[2]/classifiable_perm #REV = 97.8% (7474)
  perm_table[3]/classifiable_perm #RI = 1.2% (91)
  
  perm_table[1] 
  perm_table[2]
  perm_table[3]
  
  norm_counts_cut_param = norm_counts_cut_XY[norm_counts_cut_XY$temp_perm_calc != "UNCLASSIFIABLE",]
  dim(norm_counts_cut_param)[1]/dim(norm_counts_cut_XY)[1]
  #36.8% of genes classified
  
  #KR78
  norm_counts_resKresLDPN_param = norm_counts_cut_param[rownames(norm_counts_cut_param) %in% rownames(norm_counts_resKresLDPN_XY),]
  dim(norm_counts_resKresLDPN_param)
  table(norm_counts_resKresLDPN_param$temp_perm_calc)
  #Yeah ok so very little difference...
  #OVER = 8.0% (6)
  #REV = 89.3% (67)
  #RI = 2.7% (2)
  
  table(norm_counts_resKresLDPN_param$temp_perm_calc)[1]/nrow(norm_counts_resKresLDPN_param) #% overshooting
  table(norm_counts_resKresLDPN_param$temp_perm_calc)[2]/nrow(norm_counts_resKresLDPN_param) #% reversion
  table(norm_counts_resKresLDPN_param$temp_perm_calc)[3]/nrow(norm_counts_resKresLDPN_param) #% reinforcement
  
  table(norm_counts_resKresLDPN_param$temp_perm_calc)[1]
  table(norm_counts_resKresLDPN_param$temp_perm_calc)[2]
  table(norm_counts_resKresLDPN_param$temp_perm_calc)[3]
  dim(norm_counts_resKresLDPN_param)[1]/dim(norm_counts_resKresLDPN_XY)[1]
  #73.5% of DPN genes (that were classifiable before) are classified.
  
  #KR79
  norm_counts_resBresC_param = norm_counts_cut_param[rownames(norm_counts_cut_param) %in% rownames(norm_counts_XY_resBresC),]
  #OVER = 1.5% (2)
  #REV = 72.8% (99)
  #RI = 25.7% (35)
  
  table(norm_counts_resBresC_param$temp_perm_calc)[1]/nrow(norm_counts_resBresC_param) #% overshooting
  table(norm_counts_resBresC_param$temp_perm_calc)[2]/nrow(norm_counts_resBresC_param) #% reversion
  table(norm_counts_resBresC_param$temp_perm_calc)[3]/nrow(norm_counts_resBresC_param) #% reinforcement
  #Hmm ok so only 25% RI. 
  
  table(norm_counts_resBresC_param$temp_perm_calc)[1] #% overshooting
  table(norm_counts_resBresC_param$temp_perm_calc)[2] #% reversion
  table(norm_counts_resBresC_param$temp_perm_calc)[3] #% reinforcement
  
  #So 35 of the 91 reinforced genes are in the CEC set.
  
  dim(norm_counts_resBresC_param)[1]/dim(norm_counts_XY_resBresC)[1]
  #46.1% of genes classified before. 
  ...
  classified_vec_0.2 = rep(NA, nrow(norm_counts_cut_0.2))
  
  #Done 30/06/22
  #for (i in 1:nrow(norm_counts_cut_0.2)){
  #  Lo_row = norm_counts_cut_0.2[i,c("BD11C", "BD12C", "BD1C", "SA6C", "SA7C", "SA8C")]
  #  Lp_row = norm_counts_cut_0.2[i,c("BD11Z", "BD12Z", "BD1Z", "SA6Z", "SA7Z", "SA8Z")]
  #  La_row = norm_counts_cut_0.2[i,c("GR10Z", "GR2Z", "GR8Z", "PP1Z", "PP4Z", "PP8Z")]
  #  classified_vec_0.2[i] = plasticity_parametric_bootstrap(Lo_row, Lp_row, La_row)
  #}
  table(classified_vec_0.2) #So 13k are unclassifiable. Was tha tthe case before?
  
  norm_counts_cut_0.2$temp_perm_calc = classified_vec_0.2
  
  write.csv(classified_vec_0.2, file = "final_param_classifications_0.2.csv")
  
  #Ugghhh wtf. 
  
  perm_table_0.2 = table(norm_counts_cut_0.2$temp_perm_calc)
  classifiable_perm_0.2 = perm_table_0.2[1]+perm_table_0.2[2]+perm_table_0.2[3]
  
  #KR80
  perm_table_0.2[1]/classifiable_perm_0.2 #0.3% OVER, 98.9% REVERSION, 0.8% RI
  perm_table_0.2[2]/classifiable_perm_0.2 #31 OVER, 9720 REVERSION, 83 RI
  perm_table_0.2[3]/classifiable_perm_0.2
  
  perm_table_0.2[1]
  perm_table_0.2[2]
  perm_table_0.2[3]
  
  norm_counts_cut_param_0.2 = norm_counts_cut_0.2[norm_counts_cut_0.2$temp_perm_calc != "UNCLASSIFIABLE",]
  dim(norm_counts_cut_param_0.2)[1]/dim(norm_counts_cut_0.2)[1] #46.5% of genes from original approach.
  
  #KR81
  norm_counts_resKresLDPN_param_0.2 = norm_counts_cut_param_0.2[rownames(norm_counts_cut_param_0.2) %in% rownames(norm_counts_resKresLDPN_0.2),]
  dim(norm_counts_resKresLDPN_param_0.2)
  
  table(norm_counts_resKresLDPN_param_0.2$temp_perm_calc)[1]/nrow(norm_counts_resKresLDPN_param_0.2) #% overshooting
  table(norm_counts_resKresLDPN_param_0.2$temp_perm_calc)[2]/nrow(norm_counts_resKresLDPN_param_0.2) #% reversion
  table(norm_counts_resKresLDPN_param_0.2$temp_perm_calc)[3]/nrow(norm_counts_resKresLDPN_param_0.2) #% reinforcement
  
  table(norm_counts_resKresLDPN_param_0.2$temp_perm_calc)[1] #OVER = 4.2% (4)
  table(norm_counts_resKresLDPN_param_0.2$temp_perm_calc)[2] #REV = 91.7% (88)
  table(norm_counts_resKresLDPN_param_0.2$temp_perm_calc)[3] #RI = 4.2% (4)
  dim(norm_counts_resKresLDPN_param_0.2)[1]/dim(norm_counts_resKresLDPN_0.2)[1]
  #80.0%
  
  #KR82
  norm_counts_resBresC_param_0.2 = norm_counts_cut_param_0.2[rownames(norm_counts_cut_param_0.2) %in% rownames(norm_counts_0.2_resBresC),]
  
  table(norm_counts_resBresC_param_0.2$temp_perm_calc)[1]/nrow(norm_counts_resBresC_param_0.2) #% overshooting
  table(norm_counts_resBresC_param_0.2$temp_perm_calc)[2]/nrow(norm_counts_resBresC_param_0.2) #% reversion
  table(norm_counts_resBresC_param_0.2$temp_perm_calc)[3]/nrow(norm_counts_resBresC_param_0.2) #% reinforcement
  
  
  table(norm_counts_resBresC_param_0.2$temp_perm_calc)[1] #% overshooting 0.6% (1)
  table(norm_counts_resBresC_param_0.2$temp_perm_calc)[2] #% reversion 80.0% (136)
  table(norm_counts_resBresC_param_0.2$temp_perm_calc)[3] #% reinforcement 19.4% (33)
  
  dim(norm_counts_resBresC_param_0.2)[1]/dim(norm_counts_0.2_resBresC)[1]
  #56.7%
  ...
  ########
  norm_counts_cut_0.2IJFG = norm_counts[norm_counts$Cutoff_GC == "Y" & norm_counts$Cutoff_PC == "Y",]
  norm_counts_cut_0.2IJFG = norm_counts_cut_0.2IJFG[rownames(norm_counts_cut_0.2IJFG) %in% resI_sig$Names &
                                                      rownames(norm_counts_cut_0.2IJFG) %in% resJ_sig$Names &
                                                      rownames(norm_counts_cut_0.2IJFG) %in% resF_sig$Names &
                                                      rownames(norm_counts_cut_0.2IJFG) %in% resG_sig$Names,]
  #KR57
  dim(norm_counts_cut_0.2IJFG)[1]/dim(norm_counts)[1] #21,150 - so only 29% of genes pass this threshold
  #KR58
  dim(norm_counts_cut_XY[rownames(norm_counts_cut_XY) %in% rownames(norm_counts_cut_0.2IJFG),])[1]/dim(norm_counts_cut_XY)[1]
  
  norm_counts_noPC_0.2IJFG = norm_counts[norm_counts$Cutoff_GC == "Y" & norm_counts$Cutoff_PC == "N",]
  norm_counts_noGC_0.2IJFG = norm_counts[norm_counts$Cutoff_GC == "N" & norm_counts$Cutoff_PC == "Y",]
  
  norm_counts_cut_0.2IJFG$DirStat = norm_counts_cut_0.2IJFG$GC*norm_counts_cut_0.2IJFG$PC
  norm_counts_cut_0.2IJFG$SameDir = ifelse(norm_counts_cut_0.2IJFG$DirStat > 0, "Y", "N")
  norm_counts_cut_0.2IJFG$logPC = norm_counts_cut_0.2IJFG$PC*log2(abs(norm_counts_cut_0.2IJFG$PC))/abs(norm_counts_cut_0.2IJFG$PC)
  head(norm_counts_cut_0.2IJFG$logPC)
  head(norm_counts_cut_0.2IJFG$PC)
  norm_counts_cut_0.2IJFG$logGC = norm_counts_cut_0.2IJFG$GC*log2(abs(norm_counts_cut_0.2IJFG$GC))/abs(norm_counts_cut_0.2IJFG$GC)
  
  dim(norm_counts_cut_0.2IJFG)[1]/dim(norm_counts)[1] #...well...this is now 73% of all genes. OK. 
  norm_counts_cut_0.2IJFG$Type = ifelse(norm_counts_cut_0.2IJFG$DirStat > 0, "RI", ifelse(abs(norm_counts_cut_0.2IJFG$GC) > 0.5*abs(norm_counts_cut_0.2IJFG$PC), "REV", "OVERSHOOT"))
  
  #Looking at each population separately
  
  #2) Looking at each population individually
  norm_counts_cut_0.2IJFG_G1 = norm_counts[norm_counts$Cutoff_GC_G1 == "Y" & norm_counts$Cutoff_PC_G1 == "Y",]
  norm_counts_cut_0.2IJFG_G2 = norm_counts[norm_counts$Cutoff_GC_G2 == "Y" & norm_counts$Cutoff_PC_G2 == "Y",]
  
  norm_counts_cut_0.2IJFG_G1$DirStat = norm_counts_cut_0.2IJFG_G1$GC_G1*norm_counts_cut_0.2IJFG_G1$PC_G1
  norm_counts_cut_0.2IJFG_G2$DirStat = norm_counts_cut_0.2IJFG_G2$GC_G2*norm_counts_cut_0.2IJFG_G2$PC_G2
  
  norm_counts_cut_0.2IJFG_G1$Type = ifelse(norm_counts_cut_0.2IJFG_G1$DirStat > 0, "RI", ifelse(abs(norm_counts_cut_0.2IJFG_G1$GC_G1) > 0.5*abs(norm_counts_cut_0.2IJFG_G1$PC_G1), "REV", "OVERSHOOT"))
  norm_counts_cut_0.2IJFG_G2$Type = ifelse(norm_counts_cut_0.2IJFG_G2$DirStat > 0, "RI", ifelse(abs(norm_counts_cut_0.2IJFG_G2$GC_G2) > 0.5*abs(norm_counts_cut_0.2IJFG_G2$PC_G2), "REV", "OVERSHOOT"))
  
  100*table(norm_counts_cut_0.2IJFG_G1$Type)[1]/nrow(norm_counts_cut_0.2IJFG_G1) #% overshooting
  100*table(norm_counts_cut_0.2IJFG_G1$Type)[2]/nrow(norm_counts_cut_0.2IJFG_G1) #% reversion
  100*table(norm_counts_cut_0.2IJFG_G1$Type)[3]/nrow(norm_counts_cut_0.2IJFG_G1) #% reinforcement
  
  100*table(norm_counts_cut_0.2IJFG_G2$Type)[1]/nrow(norm_counts_cut_0.2IJFG_G2) #% overshooting
  100*table(norm_counts_cut_0.2IJFG_G2$Type)[2]/nrow(norm_counts_cut_0.2IJFG_G2) #% reversion
  100*table(norm_counts_cut_0.2IJFG_G2$Type)[3]/nrow(norm_counts_cut_0.2IJFG_G2) #% reinforcement
  
  100*dim(norm_counts_cut_0.2IJFG_G1)[1]/dim(norm_counts)[1]
  100*dim(norm_counts_cut_0.2IJFG_G2)[1]/dim(norm_counts)[1]
  
  #3) Combined transcriptome
  100*dim(norm_counts_cut_0.2IJFG)[1]/dim(norm_counts)[1] #So actually...super low? 
  
  #KR59
  100*table(norm_counts_cut_0.2IJFG$Type)[1]/nrow(norm_counts_cut_0.2IJFG) #% overshooting
  100*table(norm_counts_cut_0.2IJFG$Type)[2]/nrow(norm_counts_cut_0.2IJFG) #% reversion
  100*table(norm_counts_cut_0.2IJFG$Type)[3]/nrow(norm_counts_cut_0.2IJFG) #% reinforcement
  
  #Yup ok so....
  
  table(norm_counts_cut_0.2IJFG$Type)[1]
  table(norm_counts_cut_0.2IJFG$Type)[2]
  table(norm_counts_cut_0.2IJFG$Type)[3]
  
  head(norm_counts_cut_0.2IJFG)
  T_table_0.2IJFG = table(norm_counts_cut_0.2IJFG$Type)
  T_vec_0.2IJFG = c(0,0,0)
  T_vec_0.2IJFG[2] = table(norm_counts_cut_0.2IJFG$Type)[1] #% overshooting
  T_vec_0.2IJFG[1] = table(norm_counts_cut_0.2IJFG$Type)[2] #% reversion
  T_vec_0.2IJFG[3] = table(norm_counts_cut_0.2IJFG$Type)[3] #% reinforcement
  T_vec_0.2IJFG = data.frame(val = T_vec_0.2IJFG)
  T_vec_0.2IJFG = cbind(T_vec_0.2IJFG, type = c("REV", "OVER", "RI"))
  T_vec_0.2IJFG$type= factor(T_vec_0.2IJFG$type, levels = T_vec_0.2IJFG$type)
  
  #|FC| values for whole transcriptome
  lfcA_T_REV_0.2IJFG = lfcA[lfcA$Names %in% rownames(norm_counts_cut_0.2IJFG[norm_counts_cut_0.2IJFG$Type == "REV",]),]
  lfcE_T_REV_0.2IJFG = lfcE[lfcE$Names %in% rownames(norm_counts_cut_0.2IJFG[norm_counts_cut_0.2IJFG$Type == "REV",]),]
  lfcD_T_REV_0.2IJFG = lfcD[lfcD$Names %in% rownames(norm_counts_cut_0.2IJFG[norm_counts_cut_0.2IJFG$Type == "REV",]),]
  lfcH_T_REV_0.2IJFG = lfcH[lfcH$Names %in% rownames(norm_counts_cut_0.2IJFG[norm_counts_cut_0.2IJFG$Type == "REV",]),]
  resids_countsA_T_REV_0.2IJFG = abs(lfcA_T_REV_0.2IJFG$log2FoldChange)
  resids_countsE_T_REV_0.2IJFG = abs(lfcE_T_REV_0.2IJFG$log2FoldChange)
  resids_countsD_T_REV_0.2IJFG = abs(lfcD_T_REV_0.2IJFG$log2FoldChange)
  resids_countsH_T_REV_0.2IJFG = abs(lfcH_T_REV_0.2IJFG$log2FoldChange)
  
  lfcA_T_RI_0.2IJFG = lfcA[lfcA$Names %in% rownames(norm_counts_cut_0.2IJFG[norm_counts_cut_0.2IJFG$Type == "RI",]),]
  lfcE_T_RI_0.2IJFG = lfcE[lfcE$Names %in% rownames(norm_counts_cut_0.2IJFG[norm_counts_cut_0.2IJFG$Type == "RI",]),]
  lfcD_T_RI_0.2IJFG = lfcD[lfcD$Names %in% rownames(norm_counts_cut_0.2IJFG[norm_counts_cut_0.2IJFG$Type == "RI",]),]
  lfcH_T_RI_0.2IJFG = lfcH[lfcH$Names %in% rownames(norm_counts_cut_0.2IJFG[norm_counts_cut_0.2IJFG$Type == "RI",]),]
  resids_countsA_T_RI_0.2IJFG = abs(lfcA_T_RI_0.2IJFG$log2FoldChange)
  resids_countsE_T_RI_0.2IJFG = abs(lfcE_T_RI_0.2IJFG$log2FoldChange)
  resids_countsD_T_RI_0.2IJFG = abs(lfcD_T_RI_0.2IJFG$log2FoldChange)
  resids_countsH_T_RI_0.2IJFG = abs(lfcH_T_RI_0.2IJFG$log2FoldChange)
  
  lfcA_T_OVERSHOOT_0.2IJFG = lfcA[lfcA$Names %in% rownames(norm_counts_cut_0.2IJFG[norm_counts_cut_0.2IJFG$Type == "OVERSHOOT",]),]
  lfcE_T_OVERSHOOT_0.2IJFG = lfcE[lfcE$Names %in% rownames(norm_counts_cut_0.2IJFG[norm_counts_cut_0.2IJFG$Type == "OVERSHOOT",]),]
  lfcD_T_OVERSHOOT_0.2IJFG = lfcD[lfcD$Names %in% rownames(norm_counts_cut_0.2IJFG[norm_counts_cut_0.2IJFG$Type == "OVERSHOOT",]),]
  lfcH_T_OVERSHOOT_0.2IJFG = lfcH[lfcH$Names %in% rownames(norm_counts_cut_0.2IJFG[norm_counts_cut_0.2IJFG$Type == "OVERSHOOT",]),]
  resids_countsA_T_OVERSHOOT_0.2IJFG = abs(lfcA_T_OVERSHOOT_0.2IJFG$log2FoldChange)
  resids_countsE_T_OVERSHOOT_0.2IJFG = abs(lfcE_T_OVERSHOOT_0.2IJFG$log2FoldChange)
  resids_countsD_T_OVERSHOOT_0.2IJFG = abs(lfcD_T_OVERSHOOT_0.2IJFG$log2FoldChange)
  resids_countsH_T_OVERSHOOT_0.2IJFG = abs(lfcH_T_OVERSHOOT_0.2IJFG$log2FoldChange)
  
  resids_countsH_T_RI_0.2IJFG = as.data.frame(resids_countsH_T_RI_0.2IJFG)
  colnames(resids_countsH_T_RI_0.2IJFG) = "VALUE"
  resids_countsH_T_RI_0.2IJFG$Names = "RI"
  resids_countsH_T_REV_0.2IJFG = as.data.frame(resids_countsH_T_REV_0.2IJFG)
  colnames(resids_countsH_T_REV_0.2IJFG) = "VALUE"
  resids_countsH_T_REV_0.2IJFG$Names = "REV"
  resids_countsH_T_OVERSHOOT_0.2IJFG = as.data.frame(resids_countsH_T_OVERSHOOT_0.2IJFG)
  colnames(resids_countsH_T_OVERSHOOT_0.2IJFG) = "VALUE"
  resids_countsH_T_OVERSHOOT_0.2IJFG$Names = "OVERSHOOT"
  
  median(resids_countsH_T_RI_0.2IJFG$VALUE) #Reinforcement |FC|
  median(resids_countsH_T_REV_0.2IJFG$VALUE) #Reversion |FC|
  median(resids_countsH_T_OVERSHOOT_0.2IJFG$VALUE) #Overshooting |FC|
  
  
  Plast_FC_T_0.2IJFG = rbind(resids_countsH_T_OVERSHOOT_0.2IJFG, resids_countsH_T_REV_0.2IJFG, resids_countsH_T_RI_0.2IJFG)
  
  median(resids_countsH_T_RI_0.2IJFG$VALUE)
  median(resids_countsH_T_REV_0.2IJFG$VALUE)
  median(resids_countsH_T_OVERSHOOT_0.2IJFG$VALUE)
  
  median(resids_countsE_T_RI_0.2IJFG)
  median(resids_countsE_T_REV_0.2IJFG)
  median(resids_countsE_T_OVERSHOOT_0.2IJFG)
  
  Plast_FC_T_wilcox_0.2IJFG = pairwise.wilcox.test(Plast_FC_T_0.2IJFG$VALUE, Plast_FC_T_0.2IJFG$Names, p.adjust.method = "BH")
  Plast_FC_T_wilcox_0.2IJFG
  pairwiseMedianTest(value ~ variable, data=both_CZ_0.2IJFG) #Might be better to do this, really. 
  
  #4) reskresLDPN genes: combined
  
  dim(resKresL_DPN_sig_overlap_samedir)
  norm_counts_resKresLDPN_0.2IJFG = norm_counts_cut_0.2IJFG[rownames(norm_counts_cut_0.2IJFG) %in% resKresL_DPN_sig_overlap_samedir$Names,]
  norm_counts_resKresLDPN_0.2IJFG = norm_counts_cut_0.2IJFG[rownames(norm_counts_cut_0.2IJFG) %in% resKresL_DPN_sig_overlap_samedir$Names,]
  
  #KR62: 39% of DP(0) genes have "substantial" |PC| and |EC|
  dim(norm_counts_resKresLDPN_0.2IJFG)[1]/dim(resKresL_DPN_sig_overlap_samedir)[1]
  #KR63:
  dim(norm_counts_resKresLDPN_XY[rownames(norm_counts_resKresLDPN_0.2IJFG) %in% rownames(norm_counts_resKresLDPN_XY),])[1]/dim(norm_counts_resKresLDPN_XY)[1]
  length(unique(rownames(norm_counts_resKresLDPN_0.2IJFG), rownames(norm_counts_resKresLDPN_XY)))/dim(resKresL_DPN_sig_overlap_samedir)[1]
  
  #Whereas with the old method we do say 87%. 
  
  #KR64 (1/3) - DP(0) genes OVER=12.8%, REV=70.8%, RI=16.4%. 
  100*table(norm_counts_resKresLDPN_0.2IJFG$Type)[1]/nrow(norm_counts_resKresLDPN_0.2IJFG) #% overshooting- 14%
  100*table(norm_counts_resKresLDPN_0.2IJFG$Type)[2]/nrow(norm_counts_resKresLDPN_0.2IJFG) #% reversion - 66%
  100*table(norm_counts_resKresLDPN_0.2IJFG$Type)[3]/nrow(norm_counts_resKresLDPN_0.2IJFG) #% reinforcement - 18.7% 
  
  table(norm_counts_resKresLDPN_0.2IJFG$Type)[2]
  table(norm_counts_resKresLDPN_0.2IJFG$Type)[1]
  table(norm_counts_resKresLDPN_0.2IJFG$Type)[3]
  
  KL_Table_0.2IJFG = table(norm_counts_resKresLDPN_0.2IJFG$Type)
  KL_vec_0.2IJFG = c(0,0,0)
  KL_vec_0.2IJFG[2] = table(norm_counts_resKresLDPN_0.2IJFG$Type)[1] #% overshooting
  KL_vec_0.2IJFG[1] = table(norm_counts_resKresLDPN_0.2IJFG$Type)[2] #% reversion
  KL_vec_0.2IJFG[3] = table(norm_counts_resKresLDPN_0.2IJFG$Type)[3] #% reinforcement
  KL_vec_0.2IJFG = data.frame(val = KL_vec_0.2IJFG)
  KL_vec_0.2IJFG = cbind(KL_vec_0.2IJFG, type = c("REV", "OVER", "RI"))
  KL_vec_0.2IJFG$type= factor(KL_vec_0.2IJFG$type, levels = KL_vec_0.2IJFG$type)
  
  woof_0.2IJFG = binom.test(x = c((KL_Table_0.2IJFG[1]+KL_Table_0.2IJFG[3]), KL_Table_0.2IJFG[2]), p = (T_table_0.2IJFG[1]+T_table_0.2IJFG[3])/(T_table_0.2IJFG[1]+T_table_0.2IJFG[2]+T_table_0.2IJFG[3]), alternative = "two.sided")
  woof_0.2IJFG
  #Note - no test statistic here.
  ...
  #|FC| values for whole transcriptome
  lfcA_KL_REV_0.2IJFG = lfcA[lfcA$Names %in% rownames(norm_counts_resKresLDPN_0.2IJFG[norm_counts_resKresLDPN_0.2IJFG$Type == "REV",]),]
  lfcE_KL_REV_0.2IJFG = lfcE[lfcE$Names %in% rownames(norm_counts_resKresLDPN_0.2IJFG[norm_counts_resKresLDPN_0.2IJFG$Type == "REV",]),]
  lfcD_KL_REV_0.2IJFG = lfcD[lfcD$Names %in% rownames(norm_counts_resKresLDPN_0.2IJFG[norm_counts_resKresLDPN_0.2IJFG$Type == "REV",]),]
  lfcH_KL_REV_0.2IJFG = lfcH[lfcH$Names %in% rownames(norm_counts_resKresLDPN_0.2IJFG[norm_counts_resKresLDPN_0.2IJFG$Type == "REV",]),]
  resids_countsA_KL_REV_0.2IJFG = abs(lfcA_KL_REV_0.2IJFG$log2FoldChange)
  resids_countsE_KL_REV_0.2IJFG = abs(lfcE_KL_REV_0.2IJFG$log2FoldChange)
  resids_countsD_KL_REV_0.2IJFG = abs(lfcD_KL_REV_0.2IJFG$log2FoldChange)
  resids_countsH_KL_REV_0.2IJFG = abs(lfcH_KL_REV_0.2IJFG$log2FoldChange)
  
  lfcA_KL_RI_0.2IJFG = lfcA[lfcA$Names %in% rownames(norm_counts_resKresLDPN_0.2IJFG[norm_counts_resKresLDPN_0.2IJFG$Type == "RI",]),]
  lfcE_KL_RI_0.2IJFG = lfcE[lfcE$Names %in% rownames(norm_counts_resKresLDPN_0.2IJFG[norm_counts_resKresLDPN_0.2IJFG$Type == "RI",]),]
  lfcD_KL_RI_0.2IJFG = lfcD[lfcD$Names %in% rownames(norm_counts_resKresLDPN_0.2IJFG[norm_counts_resKresLDPN_0.2IJFG$Type == "RI",]),]
  lfcH_KL_RI_0.2IJFG = lfcH[lfcH$Names %in% rownames(norm_counts_resKresLDPN_0.2IJFG[norm_counts_resKresLDPN_0.2IJFG$Type == "RI",]),]
  resids_countsA_KL_RI_0.2IJFG = abs(lfcA_KL_RI_0.2IJFG$log2FoldChange)
  resids_countsE_KL_RI_0.2IJFG = abs(lfcE_KL_RI_0.2IJFG$log2FoldChange)
  resids_countsD_KL_RI_0.2IJFG = abs(lfcD_KL_RI_0.2IJFG$log2FoldChange)
  resids_countsH_KL_RI_0.2IJFG = abs(lfcH_KL_RI_0.2IJFG$log2FoldChange)
  
  lfcA_KL_OVERSHOOT_0.2IJFG = lfcA[lfcA$Names %in% rownames(norm_counts_resKresLDPN_0.2IJFG[norm_counts_resKresLDPN_0.2IJFG$Type == "OVERSHOOT",]),]
  lfcE_KL_OVERSHOOT_0.2IJFG = lfcE[lfcE$Names %in% rownames(norm_counts_resKresLDPN_0.2IJFG[norm_counts_resKresLDPN_0.2IJFG$Type == "OVERSHOOT",]),]
  lfcD_KL_OVERSHOOT_0.2IJFG = lfcD[lfcD$Names %in% rownames(norm_counts_resKresLDPN_0.2IJFG[norm_counts_resKresLDPN_0.2IJFG$Type == "OVERSHOOT",]),]
  lfcH_KL_OVERSHOOT_0.2IJFG = lfcH[lfcH$Names %in% rownames(norm_counts_resKresLDPN_0.2IJFG[norm_counts_resKresLDPN_0.2IJFG$Type == "OVERSHOOT",]),]
  resids_countsA_KL_OVERSHOOT_0.2IJFG = abs(lfcA_KL_OVERSHOOT_0.2IJFG$log2FoldChange)
  resids_countsE_KL_OVERSHOOT_0.2IJFG = abs(lfcE_KL_OVERSHOOT_0.2IJFG$log2FoldChange)
  resids_countsD_KL_OVERSHOOT_0.2IJFG = abs(lfcD_KL_OVERSHOOT_0.2IJFG$log2FoldChange)
  resids_countsH_KL_OVERSHOOT_0.2IJFG = abs(lfcH_KL_OVERSHOOT_0.2IJFG$log2FoldChange)
  
  norm_counts_resKresLDPN_0.2IJFG$PC_pc = norm_counts_resKresLDPN_0.2IJFG$PC/norm_counts_resKresLDPN_0.2IJFG$Lp
  norm_counts_resKresLDPN_0.2IJFG$GC_pc = norm_counts_resKresLDPN_0.2IJFG$GC/norm_counts_resKresLDPN_0.2IJFG$Lp
  
  norm_counts_noPC_resKresLDPN_0.2IJFG = norm_counts_noPC_0.2IJFG[rownames(norm_counts_noPC_0.2IJFG) %in% resKresL_DPN_sig_overlap_samedir$Names,]
  dim(norm_counts_noPC_resKresLDPN_0.2IJFG) #Note: only 18 genes.
  lfcA_KL_NOANCPLAST_0.2IJFG = lfcA[lfcA$Names %in% rownames(norm_counts_noPC_resKresLDPN_0.2IJFG),]
  lfcE_KL_NOANCPLAST_0.2IJFG = lfcE[lfcE$Names %in% rownames(norm_counts_noPC_resKresLDPN_0.2IJFG),]
  lfcD_KL_NOANCPLAST_0.2IJFG = lfcD[lfcD$Names %in% rownames(norm_counts_noPC_resKresLDPN_0.2IJFG),]
  lfcH_KL_NOANCPLAST_0.2IJFG = lfcH[lfcH$Names %in% rownames(norm_counts_noPC_resKresLDPN_0.2IJFG),]
  
  resids_countsA_KL_NOANCPLAST_0.2IJFG = abs(lfcA_KL_NOANCPLAST_0.2IJFG$log2FoldChange)
  resids_countsE_KL_NOANCPLAST_0.2IJFG = abs(lfcE_KL_NOANCPLAST_0.2IJFG$log2FoldChange)
  resids_countsD_KL_NOANCPLAST_0.2IJFG = abs(lfcD_KL_NOANCPLAST_0.2IJFG$log2FoldChange)
  resids_countsH_KL_NOANCPLAST_0.2IJFG = abs(lfcH_KL_NOANCPLAST_0.2IJFG$log2FoldChange)
  resids_countsH_KL_NOANCPLAST_0.2IJFG = as.data.frame(resids_countsH_KL_NOANCPLAST_0.2IJFG)
  colnames(resids_countsH_KL_NOANCPLAST_0.2IJFG) = "VALUE"
  resids_countsH_KL_NOANCPLAST_0.2IJFG$Names = "NOANCPLAST"
  dim(resids_countsH_KL_NOANCPLAST_0.2IJFG)
  median(resids_countsH_KL_NOANCPLAST_0.2IJFG$VALUE)
  #0.12. So the same as all the rest of them, then. 
  
  resids_countsH_KL_RI_0.2IJFG = as.data.frame(resids_countsH_KL_RI_0.2IJFG)
  colnames(resids_countsH_KL_RI_0.2IJFG) = "VALUE"
  resids_countsH_KL_RI_0.2IJFG$Names = "RI"
  resids_countsH_KL_REV_0.2IJFG = as.data.frame(resids_countsH_KL_REV_0.2IJFG)
  colnames(resids_countsH_KL_REV_0.2IJFG) = "VALUE"
  resids_countsH_KL_REV_0.2IJFG$Names = "REV"
  resids_countsH_KL_OVERSHOOT_0.2IJFG = as.data.frame(resids_countsH_KL_OVERSHOOT_0.2IJFG)
  colnames(resids_countsH_KL_OVERSHOOT_0.2IJFG) = "VALUE"
  resids_countsH_KL_OVERSHOOT_0.2IJFG$Names = "OVERSHOOT"
  
  Plast_FC_KL_0.2IJFG = rbind(resids_countsH_KL_OVERSHOOT_0.2IJFG, resids_countsH_KL_REV_0.2IJFG, resids_countsH_KL_RI_0.2IJFG)
  
  median(resids_countsH_KL_RI_0.2IJFG$VALUE)
  median(resids_countsH_KL_REV_0.2IJFG$VALUE)
  median(resids_countsH_KL_OVERSHOOT_0.2IJFG$VALUE)
  
  temp_Plast_FC_KL_0.2IJFG = Plast_FC_KL_0.2IJFG
  temp_Plast_FC_KL_0.2IJFG$Names = "PLAST"
  median(temp_Plast_FC_KL_0.2IJFG$VALUE)
  quantile(temp_Plast_FC_KL_0.2IJFG$VALUE)
  median(resids_countsH_KL_NOANCPLAST_0.2IJFG$VALUE)
  quantile(resids_countsH_KL_NOANCPLAST_0.2IJFG$VALUE)
  
  PlastNoPlast_FC_KL_0.2IJFG = rbind(temp_Plast_FC_KL_0.2IJFG, resids_countsH_KL_NOANCPLAST_0.2IJFG)
  median(temp_Plast_FC_KL_0.2IJFG$VALUE) #Plast
  median(resids_countsH_KL_NOANCPLAST_0.2IJFG$VALUE) #No Plast... Ahhhh dear, this is not the right way round...
  
  PlastNoPlast_FC_KL_wilcox_0.2IJFG = wilcox.test(temp_Plast_FC_KL_0.2IJFG$VALUE, resids_countsH_KL_NOANCPLAST_0.2IJFG$VALUE, alternative = "two.sided")
  PlastNoPlast_FC_KL_wilcox_0.2IJFG #no significant difference (insufficient numbers I guess...)
  
  KL_PlastNoPlast_Boxplot_0.2IJFG = ggplot()+
    geom_boxplot(data = PlastNoPlast_FC_KL_0.2IJFG, aes(x = Names, y=VALUE), width = 0.7, outlier.shape = NA, show.legend = FALSE)+
    #  scale_fill_manual(values = c(ctrl_col, zinc_col, ctrl_col, zinc_col, zinc_col, ctrl_col, zinc_col, ctrl_col))+
    theme(axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 15), axis.title.y = element_text(size = 20))+
    scale_x_discrete(labels = c("NP", "P"))+
    xlab("")+ylab("|FC|")+ylim(0,0.625)+
    #theme(panel.background = element_rect(fill = "lemonchiffon1"))
    theme(panel.background = element_rect(fill = NA, colour = "black", size = 2), text = element_text(size = 15))
  KL_PlastNoPlast_Boxplot_0.2IJFG
  #Should note there aren't enough here.
  
  ###################################################################
  #0.2IJFG_13D.2 AP genes only in one of the two replicates...
  ###################################################################
  
  #We already have KnotL and LnotK
  
  #Ok so for the individaul things...
  norm_counts_cut_0.2IJFG_G1_resKDPN_notLDPN = norm_counts_cut_0.2IJFG_G1[rownames(norm_counts_cut_0.2IJFG_G1) %in% resKDPN_notLDPN$Names,]
  norm_counts_cut_0.2IJFG_G2_resLDPN_notKDPN = norm_counts_cut_0.2IJFG_G2[rownames(norm_counts_cut_0.2IJFG_G2) %in% resLDPN_notKDPN$Names,]
  
  #i) what proportion of these genes don't meet the EC/PC cutoffs?
  dim(norm_counts_cut_0.2IJFG_G1_resKDPN_notLDPN) #405 meet cutoff
  dim(norm_counts_cut_0.2IJFG_G1_resKDPN_notLDPN)[1]/nrow(resKDPN_notLDPN) #So 83.5% pass both thresholds
  
  dim(norm_counts_cut_0.2IJFG_G2_resLDPN_notKDPN) #1564
  dim(norm_counts_cut_0.2IJFG_G2_resLDPN_notKDPN)/nrow(resLDPN_notKDPN) #So only 66% pass both thresholds.
  
  #ii) What proportion show substantial ancestral plasticity?
  #For this we will need to go back and look at the pre-cutoff files...
  norm_counts_G1_resKDPN_notLDPN_0.2IJFG = norm_counts[rownames(norm_counts) %in% resKDPN_notLDPN$Names,]
  norm_counts_G2_resLDPN_notKDPN_0.2IJFG = norm_counts[rownames(norm_counts) %in% resLDPN_notKDPN$Names,]
  dim(norm_counts_G1_resKDPN_notLDPN) #Same as before, good
  dim(norm_counts_G2_resLDPN_notKDPN) #Same as before, good
  
  #So for G1...
  table(norm_counts_G1_resKDPN_notLDPN_0.2IJFG$Cutoff_GC_G1) #So 437/485 pass the GC cutoff
  table(norm_counts_G1_resKDPN_notLDPN_0.2IJFG$Cutoff_GC_G1)[2]/nrow(resKDPN_notLDPN) #so that's 90%
  table(norm_counts_G1_resKDPN_notLDPN_0.2IJFG$Cutoff_PC_G1) #So 448/485.
  table(norm_counts_G1_resKDPN_notLDPN_0.2IJFG$Cutoff_PC_G1)[2]/nrow(resKDPN_notLDPN) #so 92% pass PC as well. 
  
  table(norm_counts_G2_resLDPN_notKDPN_0.2IJFG$Cutoff_GC_G2) #So 683/2365
  table(norm_counts_G2_resLDPN_notKDPN_0.2IJFG$Cutoff_GC_G2)[2]/nrow(resLDPN_notKDPN) #...71.2% show substantial GC (i.e. derived genetic change)
  table(norm_counts_G2_resLDPN_notKDPN_0.2IJFG$Cutoff_PC_G2) #So 3461/4035, or...
  table(norm_counts_G2_resLDPN_notKDPN_0.2IJFG$Cutoff_PC_G2)[2]/nrow(resLDPN_notKDPN) #but still, 92.6% show substantail PC
  
  #Ok so I think the real question is...for genes in resKDPN_notLDPN [G1 only}...do they show substantially lower anc. plast. in G2? I.e. is that why they're not DGE?
  table(norm_counts_G1_resKDPN_notLDPN_0.2IJFG$Cutoff_PC_G2)[2]/nrow(resKDPN_notLDPN) #Nope...
  table(norm_counts_G2_resLDPN_notKDPN_0.2IJFG$Cutoff_PC_G1)[2]/nrow(resLDPN_notKDPN) #Nope...
  
  #So there isn't really strong evidence here that a lack of ancestral plasticity is driving recruitment.
  
  norm_counts_cut_0.2IJFG_G1_resKresLDPN = norm_counts_cut_0.2IJFG_G1[rownames(norm_counts_cut_0.2IJFG_G1) %in% resKresL_DPN_sig_overlap_samedir$Names,]
  norm_counts_cut_0.2IJFG_G2_resKresLDPN = norm_counts_cut_0.2IJFG_G2[rownames(norm_counts_cut_0.2IJFG_G2) %in% resKresL_DPN_sig_overlap_samedir$Names,]
  dim(norm_counts_cut_0.2IJFG_G1) #very similar
  dim(norm_counts_cut_0.2IJFG_G2) #to each other
  
  #So 112 in both. I guess..conceivable?
  dim(norm_counts_cut_0.2IJFG_G1_resKresLDPN)[1]
  dim(norm_counts_cut_0.2IJFG_G2_resKresLDPN)[1]
  
  dim(norm_counts_cut_0.2IJFG_G1_resKresLDPN)[1]/nrow(resKresL_DPN_sig_overlap_samedir) #291:
  dim(norm_counts_cut_0.2IJFG_G2_resKresLDPN)[1]/nrow(resKresL_DPN_sig_overlap_samedir) #300:
  #So similar across both pairs...
  
  #SO probably some pertinent questions are:
  #Are the shared genes showing similar patterns in the individual ones.
  
  #KR74: More reversion in single population "DP(N)" genes (89.9% and 77.6%) compared to shared ones (70.8%). 
  100*table(norm_counts_cut_0.2IJFG_G1_resKDPN_notLDPN$Type)[1]/nrow(norm_counts_cut_0.2IJFG_G1_resKDPN_notLDPN) #% overshooting: 4.4
  100*table(norm_counts_cut_0.2IJFG_G1_resKDPN_notLDPN$Type)[2]/nrow(norm_counts_cut_0.2IJFG_G1_resKDPN_notLDPN) #% reversion: 89.9%
  100*table(norm_counts_cut_0.2IJFG_G1_resKDPN_notLDPN$Type)[3]/nrow(norm_counts_cut_0.2IJFG_G1_resKDPN_notLDPN) #% reinforcement; 5.7%
  table(norm_counts_cut_0.2IJFG_G1_resKDPN_notLDPN$Type)[3] #So there are 23 reinforced genes in here...
  
  100*table(norm_counts_cut_0.2IJFG_G2_resLDPN_notKDPN$Type)[1]/nrow(norm_counts_cut_0.2IJFG_G2_resLDPN_notKDPN) #% overshooting 13.1%
  100*table(norm_counts_cut_0.2IJFG_G2_resLDPN_notKDPN$Type)[2]/nrow(norm_counts_cut_0.2IJFG_G2_resLDPN_notKDPN) #% reversion 77.6%
  100*table(norm_counts_cut_0.2IJFG_G2_resLDPN_notKDPN$Type)[3]/nrow(norm_counts_cut_0.2IJFG_G2_resLDPN_notKDPN) #% reinforcement; 9.27%
  
  table(norm_counts_cut_0.2IJFG_G2_resLDPN_notKDPN$Type)[3] #And 290 reinforced genes in here.
  
  100*table(norm_counts_cut_0.2IJFG_G1_resKresLDPN$Type)[1]/nrow(norm_counts_cut_0.2IJFG_G1_resKresLDPN) #% overshooting: 11.7%
  100*table(norm_counts_cut_0.2IJFG_G1_resKresLDPN$Type)[2]/nrow(norm_counts_cut_0.2IJFG_G1_resKresLDPN) #% reversion: 69.8%
  100*table(norm_counts_cut_0.2IJFG_G1_resKresLDPN$Type)[3]/nrow(norm_counts_cut_0.2IJFG_G1_resKresLDPN) #% reinforcement: 18.5%: so way more???
  
  100*table(norm_counts_cut_0.2IJFG_G2_resKresLDPN$Type)[1]/nrow(norm_counts_cut_0.2IJFG_G2_resKresLDPN) #% overshooting: 11.8%
  100*table(norm_counts_cut_0.2IJFG_G2_resKresLDPN$Type)[2]/nrow(norm_counts_cut_0.2IJFG_G2_resKresLDPN) #% reversion: 71.6%
  100*table(norm_counts_cut_0.2IJFG_G2_resKresLDPN$Type)[3]/nrow(norm_counts_cut_0.2IJFG_G2_resKresLDPN) #% reinforcement: 16.6%...
  ...
  #13E.1 resBresC_samedir genes (CEC genes)
  ##################################################################
  norm_counts_0.2IJFG_resBresC = norm_counts_cut_0.2IJFG[rownames(norm_counts_cut_0.2IJFG) %in% resBresC_sig_overlap_samedir$Names,]
  norm_counts_0.2IJFG_resBresC = norm_counts_cut_0.2IJFG[rownames(norm_counts_cut_0.2IJFG) %in% resBresC_sig_overlap_samedir$Names,]
  
  #KR67 - CEC genes, 73% show "substantial" EC and PC. 
  dim(norm_counts_0.2IJFG_resBresC)[1]/nrow(resBresC_sig_overlap_samedir)
  dim(resBresC_sig_overlap_samedir)
  dim(norm_counts_XY_resBresC[rownames(norm_counts_XY_resBresC) %in% rownames(norm_counts_0.2IJFG_resBresC),])[1]/dim(norm_counts_XY_resBresC)[1]
  length(unique(c(rownames(norm_counts_0.2IJFG_resBresC), rownames(norm_counts_XY_resBresC))))/length(resBresC_sig_overlap_samedir$Names)
  
  
  #KR69 (1/3) - CEC genes, OVER=3.3%, REV=62.7%, RI=34%. See graphs. 
  100*table(norm_counts_0.2IJFG_resBresC$Type)[1]/(table(norm_counts_0.2IJFG_resBresC$Type)[1]+table(norm_counts_0.2IJFG_resBresC$Type)[2]+table(norm_counts_0.2IJFG_resBresC$Type)[3]) #% overshooting
  100*table(norm_counts_0.2IJFG_resBresC$Type)[2]/(table(norm_counts_0.2IJFG_resBresC$Type)[1]+table(norm_counts_0.2IJFG_resBresC$Type)[2]+table(norm_counts_0.2IJFG_resBresC$Type)[3]) #% reversion
  100*table(norm_counts_0.2IJFG_resBresC$Type)[3]/(table(norm_counts_0.2IJFG_resBresC$Type)[1]+table(norm_counts_0.2IJFG_resBresC$Type)[2]+table(norm_counts_0.2IJFG_resBresC$Type)[3]) #% reinforcement
  
  
  BC_Table_0.2IJFG
  
  BC_vec_0.2IJFG = c(0,0,0)
  BC_vec_0.2IJFG[2] = table(norm_counts_0.2IJFG_resBresC$Type)[1] #% overshooting
  BC_vec_0.2IJFG[1] = table(norm_counts_0.2IJFG_resBresC$Type)[2] #% reversion
  BC_vec_0.2IJFG[3] = table(norm_counts_0.2IJFG_resBresC$Type)[3] #% reinforcement
  BC_vec_0.2IJFG = data.frame(val = BC_vec_0.2IJFG)
  BC_vec_0.2IJFG = cbind(BC_vec_0.2IJFG, type = c("REV", "OVER", "RI"))
  BC_vec_0.2IJFG$type= factor(BC_vec_0.2IJFG$type, levels = BC_vec_0.2IJFG$type)
  
  #binom.test(x = c((BC_Table_0.2IJFG[1]+BC_Table_0.2IJFG[3]), BC_Table_0.2IJFG[2]), p = (KL_Table_0.2IJFG[1]+KL_Table_0.2IJFG[3])/(KL_Table_0.2IJFG[1]+KL_Table_0.2IJFG[2]+KL_Table_0.2IJFG[3]))
  #KR76: test for proportions of CEC vs. DP genes. 
  binom.test(x = c((BC_Table_0.2IJFG[3]), BC_Table_0.2IJFG[2]+BC_Table_0.2IJFG[1]), p = (KL_Table_0.2IJFG[3])/(KL_Table_0.2IJFG[1]+KL_Table_0.2IJFG[2]+KL_Table_0.2IJFG[3]))
  binom.test(x = c((BC_Table_0.2IJFG[3]), BC_Table_0.2IJFG[2]+BC_Table_0.2IJFG[1]), p = (T_table_0.2IJFG[3])/(T_table_0.2IJFG[1]+T_table_0.2IJFG[2]+T_table_0.2IJFG[3]))
  ...
  norm_counts_cut_G1_resBnotC_0.2IJFG = norm_counts_cut_0.2IJFG_G1[rownames(norm_counts_cut_0.2IJFG_G1) %in% resB_notC$Names,]
  norm_counts_cut_G2_resCnotB_0.2IJFG = norm_counts_cut_0.2IJFG_G2[rownames(norm_counts_cut_0.2IJFG_G2) %in% resC_notB$Names,]
  
  #i) what proportion of these genes don't meet the EC/PC cutoffs?
  dim(norm_counts_cut_G1_resBnotC_0.2IJFG) #1231
  dim(norm_counts_cut_G1_resBnotC_0.2IJFG)[1]/nrow(resB_notC) #So 68.9% pass both thresholds...
  #Update: only 37% of these genes pass the threshold, why?
  #I guess it's again because the evidence is more stringent, this is what we were seeing before.
  
  dim(norm_counts_cut_G2_resCnotB_0.2IJFG) #2630 
  dim(norm_counts_cut_G2_resCnotB_0.2IJFG)/nrow(resC_notB) #So 65.2% pass both thresholds...
  #So again now a much smaller proportion of these pass the threshold.
  ...
  norm_counts_cut_G1_resBresC_0.2IJFG = norm_counts_cut_0.2IJFG_G1[rownames(norm_counts_cut_0.2IJFG_G1) %in% resBresC_sig_overlap_samedir$Names,]
  norm_counts_cut_G2_resBresC_0.2IJFG = norm_counts_cut_0.2IJFG_G2[rownames(norm_counts_cut_0.2IJFG_G2) %in% resBresC_sig_overlap_samedir$Names,]
  dim(norm_counts_cut_G1_resBresC_0.2IJFG)[1]/nrow(resBresC_sig_overlap_samedir)#291:
  dim(norm_counts_cut_G2_resBresC_0.2IJFG)[1]/nrow(resBresC_sig_overlap_samedir) #300:
  #So similar across both pairs...
  #So you're still losing most genes here...apparently most of the reinforced ones.
  
  dim(norm_counts_cut_G1_resBnotC_0.2IJFG)[1]/nrow(resB_notC)
  dim(norm_counts_cut_G2_resCnotB_0.2IJFG)[1]/nrow(resC_notB)
  
  #SO probably some pertinent questions are:
  #Are the shared genes showing similar patterns in the individual ones.
  
  #KR73: More reversion in one sample "CEC" genes 
  100*table(norm_counts_cut_G1_resBnotC_0.2IJFG$Type)[1]/nrow(norm_counts_cut_G1_resBnotC_0.2IJFG) #% overshooting: 6.4% 
  100*table(norm_counts_cut_G1_resBnotC_0.2IJFG$Type)[2]/nrow(norm_counts_cut_G1_resBnotC_0.2IJFG) #% reversion: 77.5%
  100*table(norm_counts_cut_G1_resBnotC_0.2IJFG$Type)[3]/nrow(norm_counts_cut_G1_resBnotC_0.2IJFG) #% reinforcement; 16%
  table(norm_counts_cut_G1_resBnotC_0.2IJFG$Type)[3] #So there are 197 reinforced genes in here...
  
  100*table(norm_counts_cut_G2_resCnotB_0.2IJFG$Type)[1]/nrow(norm_counts_cut_G2_resCnotB_0.2IJFG) #% overshooting 7.14%
  100*table(norm_counts_cut_G2_resCnotB_0.2IJFG$Type)[2]/nrow(norm_counts_cut_G2_resCnotB_0.2IJFG) #% reversion 81.8%
  100*table(norm_counts_cut_G2_resCnotB_0.2IJFG$Type)[3]/nrow(norm_counts_cut_G2_resCnotB_0.2IJFG) #% reinforcement; 11.0%
  table(norm_counts_cut_G2_resCnotB_0.2IJFG$Type)[3] #And 290 reinforced genes in here.
  ...
  #So a bit of a problem here somewhere...
  #So to spell it out...
  #For the combined set of genes (0.2IJFG), you get most passing that threshold as before.
  #Considering each population separately, there's a smaller number of genes passing both thresholds.
  #That's because the power you have decreases. Only ~37-40% of genes in BnotC, CnotB and BandC, are able to be included.
  #I guess an issue is that this might be a slightly skewed set of genes.
  
  lfcA_BC_REV_0.2IJFG = lfcA[lfcA$Names %in% rownames(norm_counts_0.2IJFG_resBresC[norm_counts_0.2IJFG_resBresC$Type == "REV",]),]
  lfcE_BC_REV_0.2IJFG = lfcE[lfcE$Names %in% rownames(norm_counts_0.2IJFG_resBresC[norm_counts_0.2IJFG_resBresC$Type == "REV",]),]
  lfcD_BC_REV_0.2IJFG = lfcD[lfcD$Names %in% rownames(norm_counts_0.2IJFG_resBresC[norm_counts_0.2IJFG_resBresC$Type == "REV",]),]
  lfcH_BC_REV_0.2IJFG = lfcH[lfcH$Names %in% rownames(norm_counts_0.2IJFG_resBresC[norm_counts_0.2IJFG_resBresC$Type == "REV",]),]
  resids_countsA_BC_REV_0.2IJFG = abs(lfcA_BC_REV_0.2IJFG$log2FoldChange)
  resids_countsE_BC_REV_0.2IJFG = abs(lfcE_BC_REV_0.2IJFG$log2FoldChange)
  resids_countsD_BC_REV_0.2IJFG = abs(lfcD_BC_REV_0.2IJFG$log2FoldChange)
  resids_countsH_BC_REV_0.2IJFG = abs(lfcH_BC_REV_0.2IJFG$log2FoldChange)
  
  lfcA_BC_RI_0.2IJFG = lfcA[lfcA$Names %in% rownames(norm_counts_0.2IJFG_resBresC[norm_counts_0.2IJFG_resBresC$Type == "RI",]),]
  lfcE_BC_RI_0.2IJFG = lfcE[lfcE$Names %in% rownames(norm_counts_0.2IJFG_resBresC[norm_counts_0.2IJFG_resBresC$Type == "RI",]),]
  lfcD_BC_RI_0.2IJFG = lfcD[lfcD$Names %in% rownames(norm_counts_0.2IJFG_resBresC[norm_counts_0.2IJFG_resBresC$Type == "RI",]),]
  lfcH_BC_RI_0.2IJFG = lfcH[lfcH$Names %in% rownames(norm_counts_0.2IJFG_resBresC[norm_counts_0.2IJFG_resBresC$Type == "RI",]),]
  resids_countsA_BC_RI_0.2IJFG = abs(lfcA_BC_RI_0.2IJFG$log2FoldChange)
  resids_countsE_BC_RI_0.2IJFG = abs(lfcE_BC_RI_0.2IJFG$log2FoldChange)
  resids_countsD_BC_RI_0.2IJFG = abs(lfcD_BC_RI_0.2IJFG$log2FoldChange)
  resids_countsH_BC_RI_0.2IJFG = abs(lfcH_BC_RI_0.2IJFG$log2FoldChange)
  
  lfcA_BC_OVERSHOOT_0.2IJFG = lfcA[lfcA$Names %in% rownames(norm_counts_0.2IJFG_resBresC[norm_counts_0.2IJFG_resBresC$Type == "OVERSHOOT",]),]
  lfcE_BC_OVERSHOOT_0.2IJFG = lfcE[lfcE$Names %in% rownames(norm_counts_0.2IJFG_resBresC[norm_counts_0.2IJFG_resBresC$Type == "OVERSHOOT",]),]
  lfcD_BC_OVERSHOOT_0.2IJFG = lfcD[lfcD$Names %in% rownames(norm_counts_0.2IJFG_resBresC[norm_counts_0.2IJFG_resBresC$Type == "OVERSHOOT",]),]
  lfcH_BC_OVERSHOOT_0.2IJFG = lfcH[lfcH$Names %in% rownames(norm_counts_0.2IJFG_resBresC[norm_counts_0.2IJFG_resBresC$Type == "OVERSHOOT",]),]
  resids_countsA_BC_OVERSHOOT_0.2IJFG = abs(lfcA_BC_OVERSHOOT_0.2IJFG$log2FoldChange)
  resids_countsE_BC_OVERSHOOT_0.2IJFG = abs(lfcE_BC_OVERSHOOT_0.2IJFG$log2FoldChange)
  resids_countsD_BC_OVERSHOOT_0.2IJFG = abs(lfcD_BC_OVERSHOOT_0.2IJFG$log2FoldChange)
  resids_countsH_BC_OVERSHOOT_0.2IJFG = abs(lfcH_BC_OVERSHOOT_0.2IJFG$log2FoldChange)
  
  norm_counts_noPC_0.2IJFG_resBresC = norm_counts_noPC_0.2IJFG[rownames(norm_counts_noPC_0.2IJFG) %in% resBresC_sig_overlap_samedir$Names,]
  dim(norm_counts_noPC_0.2IJFG_resBresC)# So Out of 371, 
  
  lfcA_BC_NOANCPLAST_0.2IJFG = lfcA[lfcA$Names %in% rownames(norm_counts_noPC_0.2IJFG_resBresC),]
  lfcE_BC_NOANCPLAST_0.2IJFG = lfcE[lfcE$Names %in% rownames(norm_counts_noPC_0.2IJFG_resBresC),]
  lfcD_BC_NOANCPLAST_0.2IJFG = lfcD[lfcD$Names %in% rownames(norm_counts_noPC_0.2IJFG_resBresC),]
  lfcH_BC_NOANCPLAST_0.2IJFG = lfcH[lfcH$Names %in% rownames(norm_counts_noPC_0.2IJFG_resBresC),]
  
  resids_countsA_BC_NOANCPLAST_0.2IJFG = abs(lfcA_BC_NOANCPLAST_0.2IJFG$log2FoldChange)
  resids_countsE_BC_NOANCPLAST_0.2IJFG = abs(lfcE_BC_NOANCPLAST_0.2IJFG$log2FoldChange)
  resids_countsD_BC_NOANCPLAST_0.2IJFG = abs(lfcD_BC_NOANCPLAST_0.2IJFG$log2FoldChange)
  resids_countsH_BC_NOANCPLAST_0.2IJFG = abs(lfcH_BC_NOANCPLAST_0.2IJFG$log2FoldChange)
  resids_countsH_BC_NOANCPLAST_0.2IJFG = as.data.frame(resids_countsH_BC_NOANCPLAST_0.2IJFG)
  colnames(resids_countsH_BC_NOANCPLAST_0.2IJFG) = "VALUE"
  
  resids_countsH_BC_NOANCPLAST_0.2IJFG$Names = "NOANCPLAST"
  dim(resids_countsH_BC_NOANCPLAST_0.2IJFG)
  #SO that's more like it (woah 371 that seems like a lot)
  dim(resBresC_sig_overlap_samedir)
  #...that can't be right...
  median(resids_countsH_BC_NOANCPLAST_0.2IJFG$VALUE)
  quantile(resids_countsH_BC_NOANCPLAST_0.2IJFG$VALUE)
  #0.12. So the same as all the rest of them, then. 
  
  resids_countsH_BC_RI_0.2IJFG = as.data.frame(resids_countsH_BC_RI_0.2IJFG)
  colnames(resids_countsH_BC_RI_0.2IJFG) = "VALUE"
  resids_countsH_BC_RI_0.2IJFG$Names = "Reinforcement"
  resids_countsH_BC_REV_0.2IJFG = as.data.frame(resids_countsH_BC_REV_0.2IJFG)
  colnames(resids_countsH_BC_REV_0.2IJFG) = "VALUE"
  resids_countsH_BC_REV_0.2IJFG$Names = "REV"
  resids_countsH_BC_OVERSHOOT_0.2IJFG = as.data.frame(resids_countsH_BC_OVERSHOOT_0.2IJFG)
  colnames(resids_countsH_BC_OVERSHOOT_0.2IJFG) = "VALUE"
  resids_countsH_BC_OVERSHOOT_0.2IJFG$Names = "OVER"
  
  Plast_FC_BC_0.2IJFG = rbind(resids_countsH_BC_OVERSHOOT_0.2IJFG, resids_countsH_BC_REV_0.2IJFG, resids_countsH_BC_RI_0.2IJFG)
  
  Plast_FC_BC_wilcox_0.2IJFG = wilcox.test(resids_countsH_BC_NOANCPLAST_0.2IJFG$VALUE, Plast_FC_BC_0.2IJFG$VALUE, alternative = "two.sided")
  #KR76
  Plast_FC_BC_wilcox_0.2IJFG
  
  temp_Plast_FC_BC_0.2IJFG = Plast_FC_BC_0.2IJFG
  temp_Plast_FC_BC_0.2IJFG$Names = "PLAST"
  
  PlastNoPlast_FC_BC_0.2IJFG = rbind(resids_countsH_BC_NOANCPLAST_0.2IJFG, temp_Plast_FC_BC_0.2IJFG)
  median(temp_Plast_FC_BC_0.2IJFG$VALUE)
  median(resids_countsH_BC_NOANCPLAST_0.2IJFG$VALUE)
  
  PlastNoPlast_FC_KL_wilcox_0.2IJFG = wilcox.test(temp_Plast_FC_KL_0.2IJFG$VALUE, resids_countsH_KL_NOANCPLAST_0.2IJFG$VALUE, alternative = "two.sided")
  #KR76 (1/2)
  PlastNoPlast_FC_KL_wilcox_0.2IJFG
  PlastNoPlast_FC_BC_wilcox_0.2IJFG = wilcox.test(temp_Plast_FC_BC_0.2IJFG$VALUE, resids_countsH_BC_NOANCPLAST_0.2IJFG$VALUE, p.adjust.method = "BH")
  #KR76 (1/2)
  PlastNoPlast_FC_BC_wilcox_0.2IJFG #So there isn't any difference. Fine.
  
  #Ok so that's the BC stuff done.
  
  #6 Plotting the graphs
  
  T_vec_0.2IJFG$title = "Transcriptome-wide"
  T_vec_0.2IJFG$type = c("Reversion", "Overshooting", "Reinforcement")
  T_vec_0.2IJFG$type = factor(T_vec$type, levels = c("Reinforcement", "Overshooting", "Reversion"), ordered = T)
  
  #KR60 (1/2)
  T_barplot_0.2IJFG = ggplot(data = T_vec_0.2IJFG, aes(x = type, y = val, fill = type))+
    geom_bar(width = 0.6, show.legend = FALSE, stat = 'identity', color = "black")+xlab("")+ylab("Number of genes\n")+
    scale_fill_viridis_d()+theme(axis.text.y = element_text(size = 25),
                                 axis.text.x = element_text(size = 25), axis.title.x = element_text(size=25))+
    theme(panel.background = element_rect(fill = NA, colour = "black", size = 1, linetype = "solid"), 
  ...
  #KR51 - Barpolot for CEC genes in OVER, REV, RI
  BC_vec_0.2IJFG$title = "CEC genes"
  BC_vec_0.2IJFG$type = c("Reversion", "Overshooting", "Reinforcement")
  BC_vec_0.2IJFG$type = factor(BC_vec_0.2IJFG$type, levels = c("Reinforcement", "Overshooting", "Reversion"), ordered = T)
  BC_barplot_0.2IJFG = ggplot(data = BC_vec_0.2IJFG, aes(x = type, y = val, fill = type))+
    geom_bar(width = 0.6, show.legend = FALSE, stat = 'identity', color = "black")+xlab("")+ylab("Number of genes\n")+
    scale_fill_viridis_d()+theme(axis.text.y = element_text(size = 25),
                                 axis.text.x = element_text(size = 25), axis.title.x = element_text(size=25))+
    theme(panel.background = element_rect(fill = NA, colour = "black", size = 1, linetype = "solid"), 
  ...
  KL_vec_0.2IJFG$title = "AP genes"
  KL_vec_0.2IJFG$type = c("Reversion", "Overshooting", "Reinforcement")
  KL_vec_0.2IJFG$type = factor(KL_vec_0.2IJFG$type, levels = c("Reinforcement", "Overshooting", "Reversion"), ordered = T)
  
  #KR65 (1/2) - Barplot showing DP(0) genes classified into OVER, REV, RI
  KL_barplot_0.2IJFG = ggplot(data = KL_vec_0.2IJFG, aes(x = type, y = val, fill = type))+
    geom_bar(width = 0.6, show.legend = FALSE, stat = 'identity', color = "black")+xlab("")+ylab("Number of genes\n")+
    scale_fill_viridis_d()+theme(axis.text.y = element_text(size = 25),
                                 axis.text.x = element_text(size = 25), axis.title.x = element_text(size=25))+
    theme(panel.background = element_rect(fill = NA, colour = "black", size = 1, linetype = "solid"), 
  ...
  #KR21: Comparable levels of |FC| in genes lacking vs. having ancestral plasticity in CEC genes
  #KR57 (2/2)
  BC_PlastNoPlast_Boxplot_0.2IJFG = ggplot()+
    geom_boxplot(data = PlastNoPlast_FC_BC_0.2IJFG,fill = another_plot_purple, aes(x = Names, y=VALUE), width = 0.7, outlier.shape = NA, show.legend = FALSE)+
    #  scale_fill_manual(values = c(ctrl_col, zinc_col, ctrl_col, zinc_col, zinc_col, ctrl_col, zinc_col, ctrl_col))+
    theme(axis.text.x = element_text(size =20), axis.text.y = element_text(size = 20), axis.title.y = element_text(size = 20))+
    scale_x_discrete(labels = c("NP", "P"))+coord_cartesian(ylim = c(0,1.1))+
    #facet_grid(.~Set)+
    ...
    theme(panel.background = element_rect(fill = NA, colour = "black", size = 1, linetype = c("solid", "solid")), 
          text = element_text(size = 30),
          strip.text = element_text(size = 30))
  BC_PlastNoPlast_Boxplot_0.2IJFG
  
  #KR56 (2/2)
  KL_PlastNoPlast_Boxplot_0.2IJFG = ggplot()+
    geom_boxplot(data = PlastNoPlast_FC_KL_0.2IJFG,fill = another_plot_purple, aes(x = Names, y=VALUE), width = 0.7, outlier.shape = NA, show.legend = FALSE)+
    #  scale_fill_manual(values = c(ctrl_col, zinc_col, ctrl_col, zinc_col, zinc_col, ctrl_col, zinc_col, ctrl_col))+
    theme(axis.text.x = element_text(size =20), axis.text.y = element_text(size = 20), axis.title.y = element_text(size = 20))+
    scale_x_discrete(labels = c("NP", "P"))+
    #facet_grid(.~Set)+
  ...
  ncc_lim_0.2IJFG = max(c(quantile(norm_counts_cut_0.2IJFG$PC)[2], quantile(norm_counts_cut_0.2IJFG$PC)[4], quantile(norm_counts_cut_0.2IJFG$GC)[2], quantile(norm_counts_cut_0.2IJFG$GC)[4]))
  ap_lim_0.2IJFG = max(c(quantile(norm_counts_resKresLDPN_0.2IJFG$PC)[2], quantile(norm_counts_resKresLDPN_0.2IJFG$PC)[4], quantile(norm_counts_resKresLDPN_0.2IJFG$GC)[2], quantile(norm_counts_resKresLDPN_0.2IJFG$GC)[4]))
  cec_lim_0.2IJFG = max(c(quantile(norm_counts_0.2IJFG_resBresC$PC)[2], quantile(norm_counts_0.2IJFG_resBresC$PC)[4], quantile(norm_counts_0.2IJFG_resBresC$GC)[2], quantile(norm_counts_0.2IJFG_resBresC$GC)[4]))
  
  biggest_lim_0.2IJFG = max(c(ncc_lim_0.2IJFG, ap_lim_0.2IJFG, cec_lim_0.2IJFG))
  biggest_lim_0.2IJFG
  
  
  color_list = c("#75EAA9","#B6EA50", "#F5FF74")
  color_list
  over1_0.2IJFG = data.frame(x = c(0, -Inf, -Inf), y = c(0,0,0.5*biggest_lim_0.2IJFG*1.1))
  over2_0.2IJFG = data.frame(x = c(0, Inf, Inf), y = c(0,0,-0.5*biggest_lim_0.2IJFG*1.1))
  over3_0.2IJFG = data.frame(x = c(0, -Inf, -Inf, 0), y = c(0, 0.5*biggest_lim_0.2IJFG*1.1, Inf, Inf)) 
  over4_0.2IJFG = data.frame(x = c(0, Inf, Inf, 0), y = c(0, -0.5*biggest_lim_0.2IJFG*1.1, -Inf, -Inf)) 
  ...
  #KR61
  Whole_ECGC_0.2IJFG = ggplot(data = data.frame())+geom_point()+
    #geom_rect(aes(xmin = -Inf, xmax  = 0, ymin = -Inf, ymax = 0), fill=color_list[1])+
    #geom_rect(aes(xmin = Inf, xmax  = 0, ymin = Inf, ymax = 0), fill=color_list[1])+
    #geom_polygon(data = over1, aes(x=x, y=y), fill = color_list[2])+
    #geom_polygon(data = over2, aes(x=x, y=y), fill = color_list[2])+
    ...
    #geom_polygon(data = over4, aes(x=x, y=y), fill = color_list[3])+
    geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
    geom_bin2d(data = norm_counts_cut_0.2IJFG, aes(PC, GC), bins=50)+
    geom_abline(slope = -0.5, intercept = 0, col = "black")+
    geom_abline(slope = -1, intercept = 0, lty = 2, col = "black")+
    theme(text = element_text(size = 20))+xlab("Plastic Change (PC)")+ylab("Evolutionary Change (EC)")
  Whole_ECGC
  ...
  #KR47 
  WholeAP_ECGC_0.2IJFG = ggplot(data = data.frame())+geom_point()+
    #geom_rect(aes(xmin = -Inf, xmax  = 0, ymin = -Inf, ymax = 0), fill=color_list[1])+
    #geom_rect(aes(xmin = Inf, xmax  = 0, ymin = Inf, ymax = 0), fill=color_list[1])+
    #geom_polygon(data = over1, aes(x=x, y=y), fill = color_list[2])+
    #geom_polygon(data = over2, aes(x=x, y=y), fill = color_list[2])+
    ...
    #geom_polygon(data = over4, aes(x=x, y=y), fill = color_list[3])+
    geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
    geom_bin2d(data = norm_counts_resKresLDPN_0.2IJFG, aes(PC, GC), bins=50)+
    geom_abline(slope = -0.5, intercept = 0, col = "black")+
    geom_abline(slope = -1, intercept = 0, lty = 2, col = "black")+
    theme(text = element_text(size = 20))+xlab("Plastic Change (PC)")+ylab("Evolutionary Change (EC)")
  WholeAP_ECGC_0.2IJFG
  
  #KR52
  WholeCEC_ECGC_0.2IJFG = ggplot(data = data.frame())+geom_point()+
    #geom_rect(aes(xmin = -Inf, xmax  = 0, ymin = -Inf, ymax = 0), fill=color_list[1])+
    #geom_rect(aes(xmin = Inf, xmax  = 0, ymin = Inf, ymax = 0), fill=color_list[1])+
    #geom_polygon(data = over1, aes(x=x, y=y), fill = color_list[2])+
    #geom_polygon(data = over2, aes(x=x, y=y), fill = color_list[2])+
    ...
    #geom_polygon(data = over4, aes(x=x, y=y), fill = color_list[3])+
    geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
    geom_bin2d(data = norm_counts_0.2IJFG_resBresC, aes(PC, GC), bins=50)+
    geom_abline(slope = -0.5, intercept = 0, col = "black")+
    geom_abline(slope = -1, intercept = 0, lty = 2, col = "black")+
    theme(text = element_text(size = 20))+xlab("Plastic Change (PC)")+ylab("Evolutionary Change (EC)")
  WholeCEC_ECGC_0.2IJFG
  
  #KR60 (2/2) - yellow/green diagram for DP(0) genes, EC vs PC
  Middle_ECGC_0.2IJFG = ggplot(data = data.frame())+geom_point()+xlim(-biggest_lim_0.2IJFG, biggest_lim_0.2IJFG)+ylim(-biggest_lim_0.2IJFG, biggest_lim_0.2IJFG)+
    geom_rect(aes(xmin = -Inf, xmax  = 0, ymin = -Inf, ymax = 0), fill=color_list[1])+
    geom_rect(aes(xmin = Inf, xmax  = 0, ymin = Inf, ymax = 0), fill=color_list[1])+
    geom_polygon(data = over1, aes(x=x, y=y), fill = color_list[2])+
    geom_polygon(data = over2, aes(x=x, y=y), fill = color_list[2])+
  ...
  #KR65 (2/2)
  AP_ECGC_0.2IJFG = ggplot(data = data.frame())+geom_point()+xlim(-biggest_lim_0.2IJFG, biggest_lim_0.2IJFG)+ylim(-biggest_lim_0.2IJFG, biggest_lim_0.2IJFG)+
    geom_rect(aes(xmin = -Inf, xmax  = 0, ymin = -Inf, ymax = 0), fill=color_list[1])+
    geom_rect(aes(xmin = Inf, xmax  = 0, ymin = Inf, ymax = 0), fill=color_list[1])+
    geom_polygon(data = over1, aes(x=x, y=y), fill = color_list[2])+
    geom_polygon(data = over2, aes(x=x, y=y), fill = color_list[2])+
    ...
    geom_polygon(data = over4, aes(x=x, y=y), fill = color_list[3])+
    geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+geom_abline(slope = -0.5, intercept = 0)+
    geom_abline(slope = -1, intercept = 0, lty = 2)+geom_bin2d(data = norm_counts_resKresLDPN_0.2IJFG, aes(PC, GC), bins=50)+
    geom_hline(yintercept = -biggest_lim_0.2IJFG*1.0)+geom_hline(yintercept = biggest_lim_0.2IJFG)+
    geom_vline(xintercept = -biggest_lim_0.2IJFG)+geom_vline(xintercept = biggest_lim_0.2IJFG)+
    theme(text = element_text(size = 20))+xlab("Plastic Change (PC)")+ylab("Evolutionary Change (EC)")
  AP_ECGC_0.2IJFG
  
  #KR51 (2/2) - CEC genes green/yellow plot showing EC vs. P
  CEC_ECGC_0.2IJFG = ggplot(data = data.frame())+geom_point()+xlim(-biggest_lim_0.2IJFG, biggest_lim_0.2IJFG)+ylim(-biggest_lim_0.2IJFG, biggest_lim_0.2IJFG)+
    geom_rect(aes(xmin = -Inf, xmax  = 0, ymin = -Inf, ymax = 0), fill=color_list[1])+
    geom_rect(aes(xmin = Inf, xmax  = 0, ymin = Inf, ymax = 0), fill=color_list[1])+
    geom_polygon(data = over1, aes(x=x, y=y), fill = color_list[2])+
    geom_polygon(data = over2, aes(x=x, y=y), fill = color_list[2])+
  ...
  classified_vec_0.2IJFG = rep(NA, nrow(norm_counts_cut_0.2IJFG))
  
  #...but...hang on a second...norm_counts. That surely isn't right? s
  
#  for (i in 1:nrow(norm_counts_cut_0.2IJFG)){
#    Lo_row = norm_counts_cut_0.2IJFG[i,c("BD11C", "BD12C", "BD1C", "SA6C", "SA7C", "SA8C")]
#    Lp_row = norm_counts_cut_0.2IJFG[i,c("BD11Z", "BD12Z", "BD1Z", "SA6Z", "SA7Z", "SA8Z")]
#    La_row = norm_counts_cut_0.2IJFG[i,c("GR10Z", "GR2Z", "GR8Z", "PP1Z", "PP4Z", "PP8Z")]
#    classified_vec_0.2IJFG[i] = plasticity_parametric_bootstrap(Lo_row, Lp_row, La_row)
#  }
  table(classified_vec_0.2IJFG) #So 13k are unclassifiable. Was tha tthe case before?
  
  norm_counts_cut_0.2IJFG$temp_perm_calc = classified_vec_0.2IJFG
  
  write.csv(classified_vec_0.2IJFG, file = "final_param_classifications_0.2IJFG.csv")
  
  #Ugghhh wtf. 
  
  perm_table_0.2IJFG = table(norm_counts_cut_0.2IJFG$temp_perm_calc)
  classifiable_perm_0.2IJFG = perm_table_0.2IJFG[1]+perm_table_0.2IJFG[2]+perm_table_0.2IJFG[3]
  
  #KR83
  perm_table_0.2IJFG[1]/classifiable_perm_0.2IJFG
  perm_table_0.2IJFG[2]/classifiable_perm_0.2IJFG
  perm_table_0.2IJFG[3]/classifiable_perm_0.2IJFG
  
  perm_table_0.2IJFG[1]  #OVER 0.3% (20)
  perm_table_0.2IJFG[2] # REV 99.5% (6762)
  perm_table_0.2IJFG[3] #RI 0.2% (12)
  
  norm_counts_cut_param_0.2IJFG = norm_counts_cut_0.2IJFG[norm_counts_cut_0.2IJFG$temp_perm_calc != "UNCLASSIFIABLE",]
  dim(norm_counts_cut_param_0.2IJFG)[1]/dim(norm_counts_cut_0.2IJFG)[1] #0.82%
  
  #KR84
  norm_counts_resKresLDPN_param_0.2IJFG = norm_counts_cut_param_0.2IJFG[rownames(norm_counts_cut_param_0.2IJFG) %in% rownames(norm_counts_resKresLDPN_0.2IJFG),]
  dim(norm_counts_resKresLDPN_param_0.2IJFG)
  
  table(norm_counts_resKresLDPN_param_0.2IJFG$temp_perm_calc)[1]/nrow(norm_counts_resKresLDPN_param_0.2IJFG) #% overshooting
  table(norm_counts_resKresLDPN_param_0.2IJFG$temp_perm_calc)[2]/nrow(norm_counts_resKresLDPN_param_0.2IJFG) #% reversion
  table(norm_counts_resKresLDPN_param_0.2IJFG$temp_perm_calc)[3]/nrow(norm_counts_resKresLDPN_param_0.2IJFG) #% reinforcement
  
  table(norm_counts_resKresLDPN_param_0.2IJFG$Type)[1] # OVER=5.5% (5)
  table(norm_counts_resKresLDPN_param_0.2IJFG$Type)[2] # REV=92.2% (83)
  table(norm_counts_resKresLDPN_param_0.2IJFG$Type)[3] # RI = 2.2% (2)
  dim(norm_counts_resKresLDPN_param_0.2IJFG)[1]/dim(norm_counts_resKresLDPN_0.2IJFG)[1]
  #Only 37% of these genes were classifiable.
  
  #KR85
  norm_counts_resBresC_param_0.2IJFG = norm_counts_cut_param_0.2IJFG[rownames(norm_counts_cut_param_0.2IJFG) %in% rownames(norm_counts_0.2IJFG_resBresC),]
  
  table(norm_counts_resBresC_param_0.2IJFG$temp_perm_calc)[1]/nrow(norm_counts_resBresC_param_0.2IJFG) #% overshooting
  table(norm_counts_resBresC_param_0.2IJFG$temp_perm_calc)[2]/nrow(norm_counts_resBresC_param_0.2IJFG) #% reversion
  table(norm_counts_resBresC_param_0.2IJFG$temp_perm_calc)[3]/nrow(norm_counts_resBresC_param_0.2IJFG) #% reinforcement
  
  table(norm_counts_resBresC_param_0.2IJFG$temp_perm_calc)[1] #% overshooting 0.8% (1)
  table(norm_counts_resBresC_param_0.2IJFG$temp_perm_calc)[2] #% reversion 91.5% (119)
  table(norm_counts_resBresC_param_0.2IJFG$temp_perm_calc)[3] #% reinforcement (7.7%)
  
  dim(norm_counts_resBresC_param_0.2IJFG)[1]/dim(norm_counts_0.2IJFG_resBresC)[1]
  #Well I guess that's...interesting?
  
  #KR
  par(mfrow=c(1,1))
  pdf("~/Figure1_background_030722.pdf", width = 5.3, height = 5.84)
  color_list = c("#75EAA9","#B6EA50", "#F5FF74")
  plot(0,0, xlim = c(-10, 10), ylim = c(-10,10), bty = "n", xlab = "PC", ylab  = "EC")
  abline(h = 0, v = 0)
  abline(a = 0, b = -0.5)
  abline(a = 0, b = -1, lwd = 1, lty = 2)
  polygon(c(-10.5,-10.5, 0, 0), c(0,-10.5,-10.5,0), col = color_list[1])
  polygon(c(10.5,10.5, 0, 0), c(0,10.5,10.5,0), col = color_list[1])
  polygon(c(-10.5,-10.5, 0), c(0,5.25,0), col = color_list[2])
  polygon(c(10.5,10.5, 0), c(0,-5.25,0), col = color_list[2])
  polygon(c(-10.5,-10.5, 0, 0), c(5.25,10.5,10.5, 0), col = color_list[3])
  polygon(c(10.5,10.5, 0, 0), c(-5.25,-10.5,-10.5, 0), col = color_list[3])
...
#FFS...
#So will need to do this on a different script. 
